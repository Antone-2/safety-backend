...
const OUTPUT_PATH = outIndex >= 0 && process.argv[outIndex + 1] ? process.argv[outIndex + 1] : "";
...
async function main() {
    ...
    logger.info({ ...summary, outputPath: OUTPUT_PATH || undefined, outputFormat: OUTPUT_PATH ? (JSON_OUTPUT ? "json" : "csv") : undefined }, "Report timestamp audit completed.");
}
main()
    .catch((error) => {
    logger.error({ err: error }, "Report timestamp audit failed.");
    process.exitCode = 1;
})
    .finally(async () => {
    await pgPool.end();
