...
function requireEnv(name) {
    ...
    if (!value)
        throw new Error(`Set ${name} before running verifyAssign`);
    return value;
}
const BASE = requireEnv("VERIFY_ASSIGN_BASE_URL");
const SECRET = requireEnv("JWT_SECRET");
async function main() {
    const token = jwt.sign({ userId: "test-user-admin", email: "test.admin@crownpaints.test", name: "Test Admin", role: "super-admin" }, SECRET, { expiresIn: "1h" });
    ...
    const forReport = notifications.filter((n) => n.reportId === rid && (n.subject || "").startsWith("Task assigned"));
