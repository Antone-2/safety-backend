...
const DEFAULT_MIN_HIT_RATE_PERCENT = Number(process.env.DASHBOARD_METRICS_MIN_HIT_RATE_PERCENT || 80);
const DEFAULT_MAX_DEGRADED_COUNT = Number(process.env.DASHBOARD_METRICS_MAX_DEGRADED_COUNT || 0);
const DEFAULT_MAX_ERROR_COUNT = Number(process.env.DASHBOARD_METRICS_MAX_ERROR_COUNT || 0);
...
function getCounter(snapshot, name) {
...
async function main() {
    ...
    const hitRatePercent = totalCacheEvents > 0 ? ((hits + coalesced) / totalCacheEvents) * 100 : 0;
