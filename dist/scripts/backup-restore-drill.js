...
async function main() {
    ...
    if (!backupFile) {
        throw new Error("Usage: npm run db:restore:drill -- <backup.sql.gz|backup.sql.gz.enc>");
    }
    if (!drillDatabaseUrl) {
        throw new Error("Set RESTORE_DRILL_DATABASE_URL to a disposable database before running a restore drill.");
    }
...
main().catch((error) => {
    logger.error({ err: error }, "Backup restore drill failed");
    process.exit(1);
});
