...
const sourcePath = path.resolve(process.env.SQLITE_SOURCE_PATH || path.join(process.cwd(), "..", "data.db"));
...
function asTimestamp(value, fallback = new Date()) {
...
async function importUsers(client, db) {
    let inserted = 0;
...
async function importReports(client, db) {
    let inserted = 0;
    for (const row of rows(db, "reports")) {
        ...
        const result = await client.query(`INSERT INTO reports
...
async function importReportAudit(client, db) {
    let inserted = 0;
    for (const row of rows(db, "report_audit")) {
        const legacyId = asText(row.id);
        const result = await client.query(`INSERT INTO audit_logs
...
async function importLoginAudit(client, db) {
    let inserted = 0;
    for (const row of rows(db, "auth_login_audit")) {
        ...
        const result = await client.query(`INSERT INTO auth_login_audit
...
async function importNotifications(client, db) {
    let inserted = 0;
