    ...
    `);
        ...
        for (const row of result.rows) {
            ...
            const normalizedComplianceDueAt = row.compliance_due_at
                ...
                : null;
            const needsUpdate = row.date !== normalizedDate ||
           ...
           SET date = $2,
               ...
               compliance_due_at = $4,
               updated_at = NOW()
           WHERE id = $1`, [row.id, normalizedDate, normalizedDueAt, normalizedComplianceDueAt]);
            }
            updated += 1;
...
main()
    .catch((error) => {
    logger.error({ err: error }, "Report timestamp backfill failed.");
    process.exitCode = 1;
})
    .finally(async () => {
    await pgPool.end();
