...
const SAMPLE_LIMIT = Math.max(1, Math.min(100, Number(process.argv[process.argv.indexOf("--limit") + 1] ?? 25) || 25));
async function main() {
    if (!process.env.DATABASE_URL) {
        throw new Error("DATABASE_URL is required to audit duplicate reports in PostgreSQL.");
    }
    const totalResult = await pgPool.query("SELECT COUNT(*)::text AS count FROM reports");
    const duplicateIdsResult = await pgPool.query(`
      SELECT
        id,
        COUNT(*)::int AS copies,
      ...
      LIMIT $1
    `, [SAMPLE_LIMIT]);
    const duplicateContentResult = await pgPool.query(`
      SELECT
        date::text AS date,
        location,
        reporter,
...
main()
    .catch((error) => {
    logger.error({ err: error }, "Duplicate report audit failed.");
    process.exitCode = 1;
})
    .finally(async () => {
    await pgPool.end();
