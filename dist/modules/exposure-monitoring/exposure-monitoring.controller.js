...
import { CreateExposureMonitoringSchema, UpdateExposureMonitoringSchema, } from "./exposure-monitoring.types.js";
export function createExposureMonitoringRouter() {
    ...
    router.get("/", rbacMiddleware("exposure-monitoring:read"), async (req, res) => {
        ...
        if (exposureType)
            filters.exposureType = String(exposureType);
        if (status)
        ...
        if (riskLevel)
            filters.riskLevel = String(riskLevel);
        res.json({ data: await service.getRecords(filters) });
    });
    router.get("/:id", rbacMiddleware("exposure-monitoring:read"), async (req, res) => {
        const record = await service.getById(String(req.params.id));
        if (!record)
            throw new NotFoundError("Exposure monitoring record");
        res.json({ data: record });
    });
    router.post("/", rbacMiddleware("exposure-monitoring:create"), validate(CreateExposureMonitoringSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/:id", rbacMiddleware("exposure-monitoring:update"), validate(UpdateExposureMonitoringSchema), async (req, res) => {
    ...
    router.delete("/:id", rbacMiddleware("exposure-monitoring:delete"), async (req, res) => {
