...
function asRecord(row) {
    return {
        ...
        samplingMethod: String(row.sampling_method),
...
export class ExposureMonitoringRepository {
    ...
    async findAll(filters) {
        ...
        const result = await this.pool.query(`SELECT * FROM exposure_monitoring_records ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY sample_date DESC, created_at DESC`, params);
    ...
    async create(data) {
        ...
        const result = await this.pool.query(`INSERT INTO exposure_monitoring_records (
    ...
    async update(id, data) {
        ...
        params.push(now());
        params.push(id);
        const result = await this.pool.query(`UPDATE exposure_monitoring_records SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`, params);
        return result.rows[0] ? asRecord(result.rows[0]) : null;
    }
    async delete(id) {
    ...
    async findExceedances() {
        const result = await this.pool.query(`SELECT * FROM exposure_monitoring_records
       WHERE exceedance = TRUE
       ORDER BY sample_date DESC, created_at DESC`);
        return result.rows.map((row) => asRecord(row));
    }
    async findOverdueActions(daysBefore) {
       ...
       ORDER BY corrective_action_due_date ASC`, [future.toISOString()]);
        return result.rows.map((row) => asRecord(row));
    }
    async getStats() {
