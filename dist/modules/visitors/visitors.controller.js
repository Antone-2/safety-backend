...
export function createVisitorsRouter() {
    ...
    router.get("/", rbacMiddleware("visitors:read"), async (req, res) => {
        ...
        if (accessStatus)
            filters.accessStatus = String(accessStatus);
        if (inductionStatus)
            filters.inductionStatus = String(inductionStatus);
        if (site)
            filters.site = String(site);
        if (hostName)
            filters.hostName = String(hostName);
        if (visitDate)
            filters.visitDate = String(visitDate);
        res.json({ data: await service.getVisitors(filters) });
    });
    router.get("/:id", rbacMiddleware("visitors:read"), async (req, res) => {
        const record = await service.getById(String(req.params.id));
        if (!record)
            throw new NotFoundError("Visitor record");
        res.json({ data: record });
    });
    router.post("/", rbacMiddleware("visitors:create"), validate(CreateVisitorSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/:id", rbacMiddleware("visitors:update"), validate(UpdateVisitorSchema), async (req, res) => {
    ...
    router.delete("/:id", rbacMiddleware("visitors:delete"), async (req, res) => {
