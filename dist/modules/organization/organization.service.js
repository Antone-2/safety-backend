...
export class OrganizationService {
    ...
    async getSiteById(id) {
        ...
        if (!site)
            throw new NotFoundError("Organization site");
        return site;
    }
    async getDepartmentById(id) {
        const department = await this.repository.findDepartmentById(id);
        if (!department)
            throw new NotFoundError("Organization department");
        return department;
    }
    async createSite(input) {
        const existing = await this.repository.findSiteByName(input.name);
        if (existing)
            throw new ConflictError("Site name already exists");
    ...
    async createDepartment(input) {
        ...
        if (!created)
            throw new ValidationError("Failed to create organization department");
        return created;
    }
    async updateDepartment(id, input) {
        const existing = await this.getDepartmentById(id);
        const site = input.siteId && input.siteId !== existing.siteId
            ? await this.getSiteById(input.siteId)
            : { id: existing.siteId, name: existing.siteName };
        if (input.name || input.siteId) {
            const duplicate = await this.repository.findDepartmentByNameForSite(input.name ?? existing.name, site.name);
            if (duplicate && duplicate.id !== id) {
                throw new ConflictError("Department already exists for this site");
            }
        }
