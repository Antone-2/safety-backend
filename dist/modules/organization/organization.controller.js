...
import { OrganizationDepartmentFiltersSchema, OrganizationDepartmentSchema, OrganizationSiteFiltersSchema, OrganizationSiteSchema, UpdateOrganizationDepartmentSchema, UpdateOrganizationSiteSchema, } from "./organization.types.js";
...
export function createOrganizationRouter() {
    ...
    router.post("/sites", rbacMiddleware("users:create"), validate(OrganizationSiteSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/sites/:id", rbacMiddleware("users:update"), validate(UpdateOrganizationSiteSchema), async (req, res) => {
        ...
        await writeAuditLog({
            ...
            actor: req.user,
            request: req,
            changes: diffRecord(before, site),
        });
        res.json({ data: site });
    });
    router.delete("/sites/:id", rbacMiddleware("users:delete"), async (req, res) => {
        const before = await service.getSiteById(String(req.params.id));
    ...
    router.post("/departments", rbacMiddleware("users:create"), validate(OrganizationDepartmentSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/departments/:id", rbacMiddleware("users:update"), validate(UpdateOrganizationDepartmentSchema), async (req, res) => {
        ...
        await writeAuditLog({
            ...
            actor: req.user,
            request: req,
            changes: diffRecord(before, department),
        });
        res.json({ data: department });
    });
    router.delete("/departments/:id", rbacMiddleware("users:delete"), async (req, res) => {
        const before = await service.getDepartmentById(String(req.params.id));
