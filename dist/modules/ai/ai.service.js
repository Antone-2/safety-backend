...
function tableFromCounts(title, label, counts) {
    return {
        title,
        headers: [label, "Reports", "Share"],
        rows: counts.map(([name, count]) => [name || "Unspecified", count, ""]),
    };
}
...
function buildHtmlExport(report) {
    const rows = report.trends
        .map((row) => `<tr><td>${row.month}</td><td>${row.unsafeActs}</td><td>${row.unsafeConditions}</td><td>${row.total}</td><td>${row.highestRatedLocation ?? ""} ${row.highestRatedScore ?? ""}</td><td>${row.lowestRatedLocation ?? ""} ${row.lowestRatedScore ?? ""}</td></tr>`)
        .join("");
    return `<!doctype html><html><head><meta charset="utf-8"><title>${report.title}</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#172033}h1,h2{color:#082d63}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.kpi{border:1px solid #d0d5dd;border-radius:8px;padding:10px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #d0d5dd;padding:8px;text-align:left}th{background:#f2f4f7}</style></head><body><h1>${report.title}</h1><p>${report.executiveSummary}</p><div class="kpis">${report.kpis.map((kpi) => `<div class="kpi"><strong>${kpi.value}</strong><br>${kpi.label}<br><small>${kpi.note ?? ""}</small></div>`).join("")}</div><h2>Trend Explanation</h2>${report.interpretation.map((line) => `<p>${line}</p>`).join("")}<h2>Monthly Trend Data</h2><table><thead><tr><th>Month</th><th>Unsafe Acts</th><th>Unsafe Conditions</th><th>Total</th><th>Highest Rated</th><th>Lowest Rated</th></tr></thead><tbody>${rows}</tbody></table><h2>Recommended Actions</h2><ol>${report.recommendedActions.map((action) => `<li>${action}</li>`).join("")}</ol><h2>Sources</h2><p>${report.sources.join(", ")}</p></body></html>`;
}
export class AiService {
    ...
    async query(data, user) {
        ...
        const report = {
            ...
            sources: reports
                .slice(0, governedInput.maxSourceRecords)
        ...
        await this.savePredictionBestEffort("ai-query", governedInput, output, 0.86, user?.id);
        await this.savePromptAuditBestEffort({
            feature: "ai-query",
            prompt,
            output,
