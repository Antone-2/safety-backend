...
function asPermit(row) {
    return {
        ...
        comments: Array.isArray(row.comments) ? row.comments.map((c) => c) : [],
...
export class PermitsRepository {
    ...
    async findAll(filters) {
        ...
        return result.rows.map((row) => asPermit(row));
    }
    async findById(id) {
        const result = await this.pool.query("SELECT * FROM permits WHERE id = $1", [id]);
        return result.rows[0] ? asPermit(result.rows[0]) : null;
    }
