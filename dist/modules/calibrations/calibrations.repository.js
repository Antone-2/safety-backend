...
function asRecord(row) {
    return {
        ...
        calibrationType: String(row.calibration_type),
...
export class CalibrationsRepository {
    ...
    async findAll(filters) {
        ...
        const result = await this.pool.query(`SELECT * FROM calibration_records ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY due_date ASC, created_at DESC`, params);
    ...
    async create(data) {
        ...
        const result = await this.pool.query(`INSERT INTO calibration_records (
    ...
    async update(id, data) {
        ...
        params.push(now());
        params.push(id);
        const result = await this.pool.query(`UPDATE calibration_records SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`, params);
        return result.rows[0] ? asRecord(result.rows[0]) : null;
    }
    async delete(id) {
    ...
    async findOverdue() {
        const result = await this.pool.query(`SELECT * FROM calibration_records
       WHERE due_date < NOW()
