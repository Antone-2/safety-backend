...
import { CreateWorkplaceRegistrationSchema, UpdateWorkplaceRegistrationSchema, } from "./workplace-registration.types.js";
...
export function createWorkplaceRegistrationController(service) {
    return {
        async getRegistrations(_req, res) {
        ...
        async getRegistrationById(req, res) {
            ...
            if (!registration)
                throw new NotFoundError("Workplace registration");
        ...
        async updateRegistration(req, res) {
            ...
            if (!before)
                throw new NotFoundError("Workplace registration");
            const registration = await service.updateRegistration(String(req.params.id), req.body);
            if (!registration)
                throw new NotFoundError("Workplace registration");
            await writeAuditLog({
                ...
                resourceType: "workplace_registration",
                resourceId: registration.id,
                changes: diffRecord(before, registration),
                actor: req.user,
                request: req,
            });
            res.json({ data: registration });
