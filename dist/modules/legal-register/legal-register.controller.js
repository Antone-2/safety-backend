...
import { CreateLegalRegisterEntrySchema, UpdateLegalRegisterEntrySchema, CreateLegalObligationSchema, UpdateLegalObligationSchema, CreateObligationReviewSchema, UpdateObligationReviewSchema, CreateObligationEvidenceSchema, CreateObligationActionSchema, UpdateObligationActionSchema, } from "./legal-register.types.js";
export function createLegalRegisterController(service) {
    return {
        async getDashboard(req, res) {
            const dashboard = await service.getDashboard();
            res.json({ data: dashboard });
        },
        async getRegisterEntries(req, res) {
            const filters = {};
            const { title, legislation, jurisdiction, status } = req.query;
            if (title)
                filters.title = String(title);
            if (legislation)
                filters.legislation = String(legislation);
            if (jurisdiction)
                filters.jurisdiction = String(jurisdiction);
            if (status)
                filters.status = String(status);
            const entries = await service.getRegisterEntries(filters);
            res.json({ data: entries });
        },
        async getRegisterEntryById(req, res) {
            const entry = await service.getRegisterEntryById(String(req.params.id));
            if (!entry)
                throw new NotFoundError("Legal register entry");
            res.json({ data: entry });
        },
        async createRegisterEntry(req, res) {
            const data = req.body;
        ...
        async updateRegisterEntry(req, res) {
            ...
            await writeAuditLog({
                ...
                changes: diffRecord(before, entry),
                actor: req.user,
                request: req,
            });
            res.json({ data: entry });
        },
        async deleteRegisterEntry(req, res) {
        ...
        async getObligations(req, res) {
            ...
            if (lifecycle)
                filters.lifecycle = String(lifecycle);
            if (registerEntryId)
                filters.registerEntryId = String(registerEntryId);
        ...
        async getReviews(req, res) {
            ...
            if (obligationId)
                filters.obligationId = String(obligationId);
            if (status)
                filters.status = String(status);
        ...
        async updateReview(req, res) {
            ...
            await writeAuditLog({
                ...
                resourceType: "obligation_review",
                resourceId: String(req.params.id),
                changes: diffRecord(before, review),
                actor: req.user,
                request: req,
            });
            res.json({ data: review });
        ...
        async getEvidence(req, res) {
            ...
            if (obligationId)
                filters.obligationId = String(obligationId);
            if (reviewId)
                filters.reviewId = String(reviewId);
        ...
        async deleteEvidence(req, res) {
            ...
            if (!deleted)
                throw new NotFoundError("Obligation evidence");
            await writeAuditLog({
        ...
        async updateAction(req, res) {
            ...
            await writeAuditLog({
                ...
                resourceType: "obligation_action",
                resourceId: String(req.params.id),
                changes: diffRecord(before, action),
                actor: req.user,
                request: req,
            });
            res.json({ data: action });
