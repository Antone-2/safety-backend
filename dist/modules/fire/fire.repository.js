...
export class FireRepository {
    ...
    async findEquipment(filters) {
        ...
        return result.rows.map((row) => asFireEquipment(row));
    }
    async findEquipmentById(id) {
        const result = await this.pool.query("SELECT * FROM fire_equipment WHERE id = $1", [id]);
        return result.rows[0] ? asFireEquipment(result.rows[0]) : null;
    }
    async createEquipment(data) {
        const result = await this.pool.query(`INSERT INTO fire_equipment (id, type, location, building, floor, room, asset_tag, manufacturer, model, serial_number, installation_date, last_inspection_date, next_inspection_date, inspection_frequency, status, notes, photo_url, created_by, created_at, updated_at)
       ...
       RETURNING *`, [
        ...
        return asFireEquipment(result.rows[0]);
    }
    ...
    async findInspections(filters) {
        ...
        const result = await this.pool.query(sql, params);
        return result.rows.map((row) => asFireInspection(row));
    }
    async createInspection(data) {
        const result = await this.pool.query(`INSERT INTO fire_inspections (id, equipment_id, inspector, inspection_date, findings, defects, action_required, passed, next_inspection_due, photo_url, created_by, created_at)
       ...
       RETURNING *`, [
        ...
        const inspection = asFireInspection(result.rows[0]);
        await this.pool.query(`UPDATE fire_equipment SET last_inspection_date = $1, next_inspection_date = $2, updated_at = $3 WHERE id = $4`, [data.inspectionDate, data.nextInspectionDue, now(), data.equipmentId]);
        return inspection;
    }
    async findOverdue() {
    ...
    async getStats() {
        ...
        const defective = equipment.find((row) => row.status === "Defective")
            ? parseInt(equipment.find((row) => row.status === "Defective").count, 10)
            : 0;
