...
import { CreateInspectionSchema, CreateInspectionTemplateSchema, UpdateInspectionSchema, UpdateInspectionTemplateSchema, } from "./inspections.types.js";
export function createInspectionsRouter() {
    ...
    router.get("/templates", rbacMiddleware("inspections:read"), async (req, res) => {
        ...
        if (area)
            filters.area = String(area);
        if (site)
            filters.site = String(site);
        if (department)
            filters.department = String(department);
        if (active !== undefined)
            filters.active = String(active) === "true";
        res.json({ data: await service.getTemplates(filters) });
    });
    router.post("/templates", rbacMiddleware("inspections:create"), validate(CreateInspectionTemplateSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/templates/:id", rbacMiddleware("inspections:update"), validate(UpdateInspectionTemplateSchema), async (req, res) => {
        ...
        await writeAuditLog({
            ...
            actor: req.user,
            request: req,
            changes: diffRecord(before, template),
        });
    ...
    router.get("/", rbacMiddleware("inspections:read"), async (req, res) => {
        ...
        if (area)
            filters.area = String(area);
        if (inspector)
            filters.inspector = String(inspector);
        if (assignedTo)
    ...
    router.post("/", rbacMiddleware("inspections:create"), validate(CreateInspectionSchema), async (req, res) => {
        const data = req.body;
    ...
    router.patch("/:id", rbacMiddleware("inspections:update"), validate(UpdateInspectionSchema), async (req, res) => {
    ...
    router.delete("/:id", rbacMiddleware("inspections:delete"), async (req, res) => {
