...
function mapCapa(row) {
    return {
        ...
        reminderDays: row.reminder_days === null || row.reminder_days === undefined
            ? undefined
...
function assignmentUpdateSummary(before, after) {
    const labels = {
...
async function notifyCapaAssignment(record, options) {
    const recipients = [];
...
function summarizeAssignmentNotifications(notifications) {
...
async function getCapaNotificationHistory(id) {
    ...
    return result.rows.map((row) => ({
        ...
        recipients: Array.isArray(row.context?.notifications?.recipients)
            ? row.context.notifications.recipients.map((item) => ({
                ...
                role: item.role === "backup" || item.role === "escalation" ? item.role : "owner",
...
export function createCapaRouter() {
    ...
    router.post("/:id/notifications/resend", requirePermission("capa:update"), async (req, res) => {
        const id = routeParam(req, "id");
        const record = await getCapaById(id);
        if (!record)
            return res.status(404).json({ error: "CAPA not found" });
    ...
    router.patch("/:id/status", requirePermission("capa:update"), async (req, res) => {
        const id = routeParam(req, "id");
    ...
    router.patch("/:id", requirePermission("capa:update"), async (req, res) => {
        const id = routeParam(req, "id");
        ...
        const record = mapCapa(result.rows[0]);
        const summary = assignmentUpdateSummary(before, record);
    ...
    router.delete("/:id", requirePermission("capa:update"), async (req, res) => {
        const id = routeParam(req, "id");
    ...
    router.post("/:id/verify", requirePermission("capa:verify"), async (req, res) => {
        const id = routeParam(req, "id");
        const before = await getCapaById(id);
        if (!before)
            return res.status(404).json({ error: "CAPA not found" });
        ...
        const result = await pgPool.query(`UPDATE capa
       SET status = 'Completed',
           ...
           updated_at = NOW()
       WHERE id = $3
       RETURNING *`, [verificationNote, verifiedBy, id]);
        const record = mapCapa(result.rows[0]);
        await writeAuditLog({
    ...
    router.post("/reminders", requirePermission("capa:update"), async (req, res) => {
        ...
        const result = await pgPool.query(`SELECT id, capa_no, title, action_plan, owner, owner_email, escalation_owner, due_date, reminder_days
       FROM capa
       WHERE status NOT IN ('Cancelled', 'Completed')`);
        ...
        for (const row of result.rows) {
            ...
            const daysUntilDue = Math.round((dueStart.getTime() - nowStart.getTime()) / (24 * 60 * 60 * 1000));
            const reminderDays = row.reminder_days === null || row.reminder_days === undefined
                ? fallbackDaysBefore
                : Number(row.reminder_days);
