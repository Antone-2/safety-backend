...
export function createCapaRouter() {
    ...
    router.post("/reminders", requirePermission("capa:update"), async (req, res) => {
        ...
        const result = await pgPool.query("SELECT COUNT(*)::int AS count FROM capa WHERE status NOT IN ('Cancelled', 'Completed') AND due_date <= NOW() + ($1::int * INTERVAL '1 day')", [daysBefore]);
