...
export function createMocRouter() {
    ...
    router.get("/", rbacMiddleware("moc:read"), async (req, res) => {
        ...
        if (changeType)
            filters.changeType = String(changeType);
        if (site)
    ...
    router.patch("/:id", rbacMiddleware("moc:update"), validate(UpdateMocSchema), async (req, res) => {
    ...
    router.post("/:id/transition", rbacMiddleware("moc:approve"), validate(MocTransitionSchema), async (req, res) => {
        const before = await service.getById(String(req.params.id));
        if (!before)
            throw new NotFoundError("MOC");
    ...
    router.delete("/:id", rbacMiddleware("moc:delete"), async (req, res) => {
