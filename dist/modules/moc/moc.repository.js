...
export class MocRepository {
    ...
    async findAll(filters) {
        ...
        const result = await this.pool.query(`SELECT * FROM moc_records ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY created_at DESC`, params);
        return result.rows.map((row) => asMoc(row));
    }
    async findById(id) {
        const result = await this.pool.query("SELECT * FROM moc_records WHERE id = $1", [id]);
        return result.rows[0] ? asMoc(result.rows[0]) : null;
    }
    async create(data) {
        const mocNo = `MOC-${new Date().getFullYear()}-${String(Date.now()).slice(-5)}`;
        const result = await this.pool.query(`INSERT INTO moc_records (
    ...
    async update(id, data) {
        ...
        params.push(now());
        params.push(id);
        const result = await this.pool.query(`UPDATE moc_records SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`, params);
        return result.rows[0] ? asMoc(result.rows[0]) : null;
    }
    async delete(id) {
    ...
    async getStats() {
        const [statuses, overdue] = await Promise.all([
            ...
            this.pool.query(`SELECT COUNT(*)::int AS count FROM moc_records WHERE due_date IS NOT NULL AND due_date < NOW() AND status NOT IN ('Closed','Rejected')`),
        ]);
        const by = new Map();
