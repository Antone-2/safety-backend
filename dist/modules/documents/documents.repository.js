...
export class DocumentsRepository {
    ...
    async findAll(filters) {
        ...
        return result.rows.map((row) => asDocument(row));
    }
    async findById(id) {
        const result = await this.pool.query("SELECT * FROM documents WHERE id = $1", [id]);
        return result.rows[0] ? asDocument(result.rows[0]) : null;
    }
    async create(data) {
        const documentNo = `DOC-${new Date().getFullYear()}-${String(Date.now()).slice(-4)}`;
        const result = await this.pool.query(`INSERT INTO documents (id, title, code, category, type, version, status, content, file_url, file_name, file_size, mime_type, author, reviewer, approver, effective_date, expiry_date, site, department, tags, parent_id, created_by, document_no, owner, review_cycle_days, classification, created_at, updated_at)
       ...
       RETURNING *`, [
        ...
        return asDocument(result.rows[0]);
    }
    ...
    async createVersion(data, documentId, createdBy) {
        const result = await this.pool.query(`INSERT INTO document_versions (id, document_id, version, change_summary, content, file_url, file_name, file_size, checksum, created_by, created_at)
       ...
       RETURNING *`, [
        ...
        return asVersion(result.rows[0]);
    }
    async findVersions(documentId) {
        ...
        return result.rows.map((row) => asVersion(row));
    }
    async createApproval(approval) {
        const result = await this.pool.query(`INSERT INTO document_approvals (id, document_id, version, step, status, approver_id, approver_name, comments, decided_at, created_at)
       ...
       RETURNING *`, [
        ...
        return asApproval(result.rows[0]);
    }
    async findApprovals(documentId) {
        ...
        return result.rows.map((row) => asApproval(row));
    }
    async createAcknowledgement(ack) {
        const result = await this.pool.query(`INSERT INTO document_acknowledgements (id, document_id, document_version, user_id, user_email, user_name, acknowledged_at, ip_address, user_agent)
    ...
    async findAcknowledgements(documentId) {
    ...
    async createAccessLink(link) {
       ...
       RETURNING *`, [
        ...
        return asAccessLink(result.rows[0]);
    }
    async getStats() {
        ...
        const dueResult = await this.pool.query("SELECT COUNT(*) as count FROM documents WHERE next_review_date IS NOT NULL AND next_review_date <= NOW()");
