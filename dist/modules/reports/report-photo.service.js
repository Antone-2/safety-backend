...
function extractDriveFileId(value) {
    ...
    for (const candidate of candidates) {
        const match = candidate.match(DRIVE_FILE_RE) ||
...
export function buildDriveFetchCandidates(fileId) {
...
export function buildReportPhotoStorageKey(reportId, contentType, originalName) {
...
export async function storeReportPhotoFromDrive(reportId, sourceUrl, originalName) {
    ...
    if (isS3StorageConfigured()) {
        try {
            ...
            logger.warn({ err: error, reportId, sourceUrl: rawUrl }, "Failed to upload report photo to object storage");
        }
    }
    else {
        logger.debug({ reportId, sourceUrl: rawUrl }, "S3 storage not configured; storing report photo in database fallback");
    }
    ...
    try {
        await client.query(`INSERT INTO report_photos (report_id, drive_file_id, original_name, content_type, data, source_url, storage_key, storage_url)
       ...
       ON CONFLICT (report_id) DO UPDATE SET
        ...
        logger.warn({ err: error, reportId }, "Failed to store report photo");
        return false;
    }
    finally {
        client.release();
...
export async function getStoredReportPhoto(reportId) {
    const client = await getDbClient();
    try {
        const result = await client.query("SELECT data, content_type, storage_key FROM report_photos WHERE report_id = $1", [reportId]);
