...
function monthKeysBetween(start, count) {
...
export async function getMonthlyEhsReport(month) {
    const { month: monthKey, start, next, trendStart } = parseMonth(month);
    const monthKeys = monthKeysBetween(trendStart, 12);
    const [incidentResult, capaResult, trainingResult, complianceResult, auditResult, legalUpdateResult, emissionResult, spillResult, healthResult,] = await Promise.all([
        pgPool.query(`SELECT id, date, type, location, severity, status, department, description
       FROM reports
       WHERE date >= $1 AND date < $2
       ORDER BY date DESC
       LIMIT 5000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, title, owner, priority, status, due_date, completed_date, created_at
       FROM capa
       WHERE created_at < $2
         AND (
           created_at >= $1
       ...
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT r.id, r.status, r.department, r.scheduled_date, r.completed_date, r.expiry_date, c.title AS course_title
       ...
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, title, status, due_date, created_at
       ...
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, title, status, start_date, end_date, created_at
       FROM audits
       WHERE created_at < $2
         AND (
           start_date >= $1
       ...
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, title, status, due_date, effective_date, created_at
       FROM legal_updates
       WHERE created_at < $2
         AND (
           effective_date >= $1
       ...
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, status, monitored_date
       FROM emissions
       WHERE monitored_date >= $1 AND monitored_date < $2
       ORDER BY monitored_date DESC
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, severity, date, cleanup_completed, reported_to_nema
       FROM spills
       WHERE date >= $1::date AND date < $2::date
       ORDER BY date DESC
       LIMIT 2000`, [trendStart.toISOString(), next.toISOString()]),
        pgPool.query(`SELECT id, next_due_date, fitness_for_work, type, examination_date
    ...
    const allIncidents = incidentResult.rows.map((row) => asIncident(row));
    const incidents = allIncidents.filter((item) => item.date >= start.toISOString() && item.date < next.toISOString());
    const trendMap = new Map(monthKeys.map((key) => [key, { month: toMonthLabel(key), incidents: 0, closed: 0, open: 0 }]));
    ...
    const sites = Array.from(siteMap.entries())
        .map(([name, data]) => {
        const riskScore = data.maxSeverity >= 4 || data.count > 10
            ? "High"
            : data.maxSeverity >= 3 || data.count > 5
                ? "Medium"
                : "Low";
        const trend = data.maxSeverity >= 4 || data.count > 10
            ? "Needs attention"
            : data.maxSeverity >= 3 || data.count > 5
                ? "Watch"
                : "Stable";
    ...
    const selectedCapas = capaResult.rows
        ...
        const relevant = dueBeforeMonthEnd ||
        ...
        .slice(0, 10);
    const capaTrendMap = new Map(monthKeys.map((key) => [key, { month: toMonthLabel(key), opened: 0, closed: 0, overdue: 0 }]));
    let overdueCapas = 0;
    ...
    for (const row of trainingResult.rows) {
        ...
        if (expiryDate < monthEndPlus30.toISOString())
            existing.expiryDate = expiryDate;
        if (status === "Expired" || expiryDate < next.toISOString())
    ...
    const training = Array.from(trainingGroups.values())
        ...
        .sort((left, right) => {
        const order = { Expired: 0, "Expiring soon": 1, "On track": 2 };
    ...
    const totalTrainingCompleted = Array.from(trainingGroups.values()).reduce((sum, group) => sum + group.completed, 0);
    ...
    for (const row of auditResult.rows) {
        ...
        if (activeAuditStatuses.has(status))
            openAudits += 1;
    }
    ...
    const emissionExceedances = currentMonthEmissions.filter((row) => String(row.status ?? "Within Limit") !== "Within Limit").length;
    ...
    const majorSpills = currentMonthSpills.filter((row) => ["Major", "Critical"].includes(String(row.severity ?? "Minor"))).length;
    ...
    for (const row of healthResult.rows) {
        ...
        if (nextDueDate < monthEndPlus30Iso)
            healthDueSoon += 1;
        if (!row.fitness_for_work)
            healthRestrictions += 1;
    }
    ...
    const currentFocus = criticalOpen > 0
