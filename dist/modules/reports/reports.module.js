...
function parseInsightTime(value) {
    if (!value)
        return null;
    const time = new Date(value).getTime();
    return Number.isFinite(time) ? time : null;
}
function countInRange(reports, fromInclusive, toExclusive) {
...
function buildDashboardInsights(reports) {
    const now = new Date();
    ...
    const lastHighOrCritical = reports
        ...
        .sort((a, b) => b - a)[0];
    const closedWithResolution = reports.filter((report) => report.status === "Closed" && report.resolutionDays != null);
    const closedWithinSla = closedWithResolution.filter((report) => typeof report.resolutionDays === "number" &&
        typeof report.slaHours === "number" &&
        report.resolutionDays * 24 <= report.slaHours).length;
    return {
        rolling30Days: {
        ...
        daysSinceLastHighOrCritical: typeof lastHighOrCritical === "number"
...
function assignmentScreenUrl(req, id) {
    ...
    if (configuredFrontend)
        return new URL(path, configuredFrontend).toString();
...
export function createReportsRouter() {
    ...
    router.get("/monthly-ehs", authenticateUser, requirePermission("reports:read"), async (req, res) => {
        try {
    ...
    router.get("/top-reporters-mtd", authenticateUser, requirePermission("reports:read"), async (req, res) => {
    ...
    router.get("/leaderboard", authenticateUser, requirePermission("reports:read"), async (req, res) => {
        ...
        try {
            setCachedJsonHeaders(res, REPORTS_DASHBOARD_CACHE_TTL_SECONDS);
            const leaderboard = await cacheService.getOrSet(cacheKey, () => reportsService.getLeaderboard(requestedMonth || undefined, requestedReporter || undefined), REPORTS_DASHBOARD_CACHE_TTL_SECONDS);
    ...
    router.get("/leaderboard/by-location", authenticateUser, requirePermission("reports:read"), async (req, res) => {
        const cacheKey = buildReportsCacheKey("leaderboard-by-location", {
            role: req.user?.role ?? "unknown",
        });
        try {
            setCachedJsonHeaders(res, REPORTS_DASHBOARD_CACHE_TTL_SECONDS);
            const data = await cacheService.getOrSet(cacheKey, () => reportsService.getLeaderboardByLocation(), REPORTS_DASHBOARD_CACHE_TTL_SECONDS);
            res.json({ data });
        }
        catch (error) {
    ...
    router.get("/generate", authenticateUser, requirePermission("reports:read"), async (req, res) => {
        try {
    ...
    router.get("/:id/photo", async (req, res) => {
        ...
        const size = Math.max(100, Math.min(1920, Number.isFinite(rawSize) ? rawSize : 400));
        ...
        try {
            ...
            const { getStoredReportPhoto, storeReportPhotoFromDrive } = await import("./report-photo.service.js");
    ...
    router.post("/", authenticateUser, requirePermission("reports:create"), async (req, res) => {
    ...
    router.patch("/:id/status", authenticateUser, requirePermission("reports:update"), async (req, res) => {
        const id = routeParam(req, "id");
    ...
    router.patch("/:id/assign", authenticateUser, requirePermission("reports:assign"), async (req, res) => {
        ...
        const assignedToCopy = Array.from(new Set(parsed.data.assignedToCopy.filter((email) => email !== assignedTo)));
        ...
        try {
            ...
            await assignmentsService.enqueueReportAssignmentSync({
                ...
                reason: typeof req.body?.assignmentReason === "string" ? req.body.assignmentReason.trim() : undefined,
                dueAt: updated.dueAt,
                priority: ["Low", "Medium", "High", "Critical"].includes(String(updated.severity)) ? String(updated.severity) : undefined,
            }, req, error);
        }
        await invalidateReportsCache();
    ...
    router.post("/:id/closure-requests", authenticateUser, async (req, res) => {
        ...
        const photos = Array.isArray(req.body?.photos)
            ? req.body.photos.map((photo) => String(photo)).filter(Boolean)
            : [];
        if (!resolutionNotes) {
        ...
        try {
            const { createClosureRequest } = await import("../../services/report-closure.service.js");
    ...
    router.post("/:id/closure-requests/:requestId/approve", authenticateUser, requirePermission("reports:approve"), async (req, res) => {
        ...
        try {
            const { approveClosureRequest } = await import("../../services/report-closure.service.js");
    ...
    router.post("/:id/closure-requests/:requestId/reject", authenticateUser, requirePermission("reports:approve"), async (req, res) => {
        const reportId = routeParam(req, "id");
        const requestId = routeParam(req, "requestId");
        const reviewedBy = String(req.user?.name || req.user?.email || "");
        const reviewedByEmail = String(req.user?.email || "");
        ...
        try {
            const { rejectClosureRequest } = await import("../../services/report-closure.service.js");
    ...
    router.get("/:id/closure-requests", authenticateUser, async (req, res) => {
        const id = routeParam(req, "id");
        try {
            const { listClosureRequests } = await import("../../services/report-closure.service.js");
    ...
    router.post("/:id/comments", authenticateUser, async (req, res) => {
        const id = routeParam(req, "id");
        const author = String(req.body?.author || req.user?.name || req.user?.email || "");
