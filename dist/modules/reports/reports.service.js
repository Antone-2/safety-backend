...
import { GOOGLE_SHEETS_TIMEZONE, getStartOfSheetDayUtc, getStartOfSheetMonthUtc, sanitizeReportDate, tryParseReportDateWithFallbacks, } from "../../shared/utils/report-date.js";
...
let pgReportColumnsPromise = null;
const DASHBOARD_ANALYTICS_SAMPLE_LIMIT = Math.max(50, Math.min(200, Number(process.env.REPORTS_DASHBOARD_ANALYTICS_LIMIT || 150) || 150));
export function buildLeaderboardFilter(filters) {
    const where = [];
    const params = [];
    if (filters.month) {
...
export function buildPgFilter(filters) {
    const where = [`${REPORT_LOCAL_DATE_SQL} <= ${CURRENT_REPORT_LOCAL_DATE_SQL}`];
    const params = [];
    ...
    if (filters.dateFrom) {
        add((p) => `date >= ${p}::TIMESTAMPTZ`, filters.dateFrom);
    }
    if (filters.dateTo) {
        add((p) => `date < ${p}::TIMESTAMPTZ`, filters.dateTo);
    }
    if (!filters.dateFrom && filters.days && String(filters.days) !== "9999") {
        add((p) => `date >= ${p}`, new Date(Date.now() - Number(filters.days) * 86400000).toISOString());
    }
    if (filters.search) {
        params.push(`%${filters.search}%`);
        const p = `$${params.length}`;
        where.push(`(description ILIKE ${p} OR reporter ILIKE ${p} OR id ILIKE ${p} OR location ILIKE ${p} OR category ILIKE ${p})`);
    }
    return { whereSql: where.join(" AND "), params };
}
export function buildSqliteFilter(filters) {
    const where = ["date < ?"];
    const params = [getStartOfSheetDayUtc(new Date(), 1).toISOString()];
    const add = (clause, value) => {
        where.push(clause);
        params.push(value);
    };
    ...
    if (filters.dateFrom)
        add("date >= ?", filters.dateFrom);
    if (filters.dateTo)
        add("date < ?", filters.dateTo);
    if (!filters.dateFrom && filters.days && String(filters.days) !== "9999") {
        add("date >= ?", new Date(Date.now() - Number(filters.days) * 86400000).toISOString());
    }
    if (filters.search) {
        where.push("(description LIKE ? OR reporter LIKE ? OR id LIKE ? OR location LIKE ? OR category LIKE ?)");
        const search = `%${filters.search}%`;
        params.push(search, search, search, search, search);
    }
    return { whereSql: where.join(" AND "), params };
}
async function writeAuditLogBestEffort(input) {
    try {
        await writeAuditLog(input);
    }
    catch (error) {
        console.warn("Audit log write skipped:", error instanceof Error ? error.message : String(error));
    }
}
...
function mapReport(row, comments = [], audit = []) {
    if (!row || !row.id)
        return null;
...
function assignmentActorEmail(request) {
    return normalizeEmail(request?.user?.email || request?.body?.assignedBy);
}
async function recordAssignmentNotifications(reportId, notifications) {
    if (!notifications.length)
        return;
    try {
        const db = await getDb();
        const now = new Date().toISOString();
        const insert = db.prepare("INSERT INTO notifications (id, reportId, channel, recipient, subject, message, delivered, createdAt, read) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        console.warn("Assignment notification persistence skipped:", error instanceof Error ? error.message : String(error));
    }
}
async function syncReportWorkflowState(input) {
    if (!isPgAvailable())
        return;
    try {
        await pgPool.query(`INSERT INTO workflow_instances (resource_type, resource_id, workflow_name, state, assigned_to, due_at, context)
...
function normalizeReportInput(input) {
    return {
        ...
        isLostTimeInjury: (input.isLostTimeInjury ?? input.is_lost_time_injury) ? 1 : 0,
        medicalTreatmentCase: (input.medicalTreatmentCase ?? input.medical_treatment_case) ? 1 : 0,
        lostWorkDays: Number(input.lostWorkDays ?? input.lost_work_days ?? 0),
        restrictedWorkDays: Number(input.restrictedWorkDays ?? input.restricted_work_days ?? 0),
        classificationSource: input.classificationSource ?? input.classification_source,
        classificationVerifiedBy: input.classificationVerifiedBy ?? input.classification_verified_by,
        classificationVerifiedAt: input.classificationVerifiedAt ?? input.classification_verified_at,
...
export class ReportsService {
    async list(filters = {}, page = 1, limit = 50) {
        ...
        if (isPgAvailable()) {
            try {
                ...
                console.warn("Reports list failed in PostgreSQL-only mode:", error instanceof Error ? error.message : String(error));
                throw error;
            }
        }
        return this.listSqlite(filters, page, limit);
    }
    async listPg(filters, page, limit) {
        const { whereSql, params } = buildPgFilter(filters);
        const offset = (page - 1) * limit;
        const totalResult = await pgPool.query(`SELECT COUNT(*)::int AS total FROM reports WHERE ${whereSql}`, params);
        const rows = filters.all
            ? await pgPool.query(`SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT $${params.length + 1}`, [...params, 500])
            : await pgPool.query(`SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, limit, offset]);
        return {
            data: rows.rows.map((row) => mapReport(row)).filter((row) => row !== null),
            total: totalResult.rows[0]?.total ?? 0,
            page,
            limit: filters.all ? rows.rows.length : limit,
        };
    }
    async listSqlite(filters, page, limit) {
        const db = await getDb();
        const { whereSql, params } = buildSqliteFilter(filters);
        const offset = (page - 1) * limit;
        const totalRow = allRows(db, `SELECT COUNT(*) as total FROM reports WHERE ${whereSql}`, params)[0];
        const rows = filters.all
            ? allRows(db, `SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT ?`, [...params, 500])
            : allRows(db, `SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
        return {
            data: rows.map((row) => mapReport(row)).filter((row) => row !== null),
            total: Number(totalRow?.total ?? 0),
            page,
            limit: filters.all ? rows.length : limit,
        };
    }
    async listDashboardAnalytics(filters = {}, maxRows = DASHBOARD_ANALYTICS_SAMPLE_LIMIT) {
        const safeLimit = Math.max(1, Math.min(500, Math.trunc(maxRows) || DASHBOARD_ANALYTICS_SAMPLE_LIMIT));
        if (isPgAvailable()) {
            const { whereSql, params } = buildPgFilter(filters);
            const rows = await pgPool.query(`SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT $${params.length + 1}`, [...params, safeLimit]);
            return rows.rows
        ...
        const db = await getDb();
        const { whereSql, params } = buildSqliteFilter(filters);
        const rows = allRows(db, `SELECT * FROM reports WHERE ${whereSql} ORDER BY date DESC LIMIT ?`, [...params, safeLimit]);
        return rows
            .map((row) => mapReport(row))
            .filter((row) => row !== null);
    }
    ...
    async getByIdPg(id) {
        const rowResult = await pgPool.query("SELECT * FROM reports WHERE id = $1", [id]);
        const row = rowResult.rows[0];
        if (!row)
            return null;
        const [comments, audit] = await Promise.all([
            pgPool.query("SELECT author, text, created_at FROM report_comments WHERE report_id = $1 ORDER BY created_at ASC", [id]),
            pgPool.query("SELECT actor_id, actor_email, action, context, created_at FROM audit_logs WHERE resource_type = 'report' AND resource_id = $1 ORDER BY created_at DESC", [id]),
        ]);
        return mapReport(row, comments.rows, audit.rows);
    }
    async getByIdSqlite(id) {
        const db = await getDb();
        const rows = allRows(db, "SELECT * FROM reports WHERE id = ?", [id]);
        const row = rows[0];
        if (!row)
            return null;
        const comments = allRows(db, "SELECT author, text, at FROM comments WHERE reportId = ? ORDER BY at ASC", [id]);
        const audit = allRows(db, "SELECT actor, action, detail, createdAt FROM report_audit WHERE reportId = ? ORDER BY createdAt DESC", [id]);
    ...
    async createPg(input, request) {
        ...
        const complianceRequired = Boolean(input.complianceRequired ||
    ...
    async createSqlite(input, request) {
        const db = await getDb();
        const id = `RPT-${String(Date.now()).slice(-5)}`;
        const now = new Date();
        const slaHours = input.severity === "Critical" ? 24 : input.severity === "High" ? 72 : 168;
    ...
    async updateStatus(id, status, request) {
        if (isPgAvailable()) {
            return this.updateStatusPg(id, status, request);
        }
        return this.updateStatusSqlite(id, status, request);
    }
    async updateStatusPg(id, status, request) {
        const before = await this.getById(id);
        const row = await pgPool.query("UPDATE reports SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *", [status, id]);
        if (!row.rows[0])
            return null;
        if (request) {
            await writeAuditLogBestEffort({
    ...
    async updateStatusSqlite(id, status, request) {
        const db = await getDb();
        const before = await this.getById(id);
        const rows = allRows(db, "UPDATE reports SET status = ?, updatedAt = ? WHERE id = ? RETURNING *", [status, new Date().toISOString(), id]);
        const row = rows[0];
        if (!row)
            return null;
        if (request) {
    ...
    async updateAssignment(id, assignedTo, assignedToCopy, request) {
        if (isPgAvailable()) {
            try {
                return await this.updateAssignmentPg(id, assignedTo, assignedToCopy, request);
            }
            catch (error) {
                console.warn("PostgreSQL assignment update failed in PostgreSQL-only mode:", error instanceof Error ? error.message : String(error));
                throw error;
            }
        }
        return this.updateAssignmentSqlite(id, assignedTo, assignedToCopy, request);
    }
    async notifyAssignment(report, assignedTo, assignedToCopy, request) {
        ...
        const notifications = await sendReportAssignmentNotifications(report, recipients, assignedBy, primaryEmail || assignedTo);
    ...
    async updateAssignmentPg(id, assignedTo, assignedToCopy, request) {
        ...
        if (!updated)
            return null;
        const assignmentNotifications = await this.notifyAssignment(updated, assignedTo, assignedToCopy, request);
        await scheduleFollowupsForReport(id).catch(() => { });
        return { ...updated, assignmentNotifications };
    }
    async updateAssignmentSqlite(id, assignedTo, assignedToCopy, request) {
        const db = await getDb();
        const row = db
    ...
    async addComment(id, author, text, request) {
        if (isPgAvailable()) {
            return this.addCommentPg(id, author, text, request);
        }
        return this.addCommentSqlite(id, author, text, request);
    }
    async addCommentPg(id, author, text, request) {
        const exists = await pgPool.query("SELECT id FROM reports WHERE id = $1", [
            id,
        ]);
        if (!exists.rows[0])
            return null;
        await pgPool.query("INSERT INTO report_comments (report_id, author, text) VALUES ($1, $2, $3)", [id, author, text]);
        if (request) {
            await writeAuditLogBestEffort({
    ...
    async addCommentSqlite(id, author, text, request) {
        const db = await getDb();
        const rows = allRows(db, "SELECT id FROM reports WHERE id = ?", [id]);
        if (!rows[0])
            return null;
        const commentId = `cmt-${randomBytes(6).toString("hex")}`;
        db.run("INSERT INTO comments (id, reportId, author, text, at) VALUES (?, ?, ?, ?, ?)", [commentId, id, author, text, new Date().toISOString()]);
        if (request) {
            await writeAuditLogBestEffort({
                action: "report.comment.added",
                resourceType: "report",
    ...
    async updatePg(id, fields, request) {
        ...
        if (sets.length === 0)
            return this.getById(id);
        params.push(id);
        const updatedRow = await pgPool.query(`UPDATE reports SET ${sets.join(", ")}, updated_at = NOW() WHERE id = $${idx} RETURNING *`, params);
    ...
    async updateSqlite(id, fields, request) {
        const db = await getDb();
        const allowed = {
            location: "location",
            reporter: "reporter",
        ...
        if (!existing)
            return null;
        db.prepare(`UPDATE reports SET ${sets.join(", ")}, updatedAt = ? WHERE id = ?`).run(params);
        await saveDb(db);
        if (request) {
    ...
    async deletePg(id, request) {
        const deleted = await pgPool.query("DELETE FROM reports WHERE id = $1 RETURNING id", [id]);
        if (!deleted.rows[0])
            return null;
        if (request) {
            await writeAuditLogBestEffort({
    ...
    async getPhotoUrl(id) {
        if (isPgAvailable()) {
            const result = await pgPool.query("SELECT photo_url FROM reports WHERE id = $1", [id]);
            const row = result.rows[0];
            if (!row)
                return { photoUrl: "", found: false };
    ...
    async stats() {
      ...
      `);
        ...
        const total = Number(allRows(db, "SELECT COUNT(*) as c FROM reports")[0]?.c ?? 0);
        const open = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE status = 'Open'")[0]?.c ?? 0);
        const closed = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE status = 'Closed'")[0]?.c ?? 0);
        const todayCount = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE date >= ? AND date < ?", [
            today,
            tomorrow,
        ])[0]?.c ?? 0);
        const weekCount = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE date >= ?", [
            weekAgo,
        ])[0]?.c ?? 0);
        const avgResolution = Number(allRows(db, "SELECT AVG(resolutionDays) as avg FROM reports WHERE status = 'Closed' AND resolutionDays IS NOT NULL")[0]?.avg ?? 0);
        return {
            total,
            open,
            closed,
    ...
    async summary(filters = {}) {
        ...
        const periodMultiplier = filters.days && String(filters.days) !== "9999"
            ? Math.max(Number(filters.days) / 30, 1 / 30)
            : 12;
        const totalManhoursWorked = Math.round(totalWorkforce * dailyWorkHours * workDaysPerMonth * periodMultiplier);
        if (isPgAvailable()) {
            const { whereSql, params } = buildPgFilter(filters);
            const nowParam = `$${params.length + 1}`;
            const reportColumns = await getPgReportColumns();
            const hasColumn = (name) => reportColumns.has(name);
    ...
    async topReportersMonthToDate(limit = 6) {
    ...
    async legacySummary() {
      ...
      `);
        ...
        const criticalOpen = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE severity = 'Critical' AND status != 'Closed'")[0]?.c ?? 0);
        const overdue = Number(allRows(db, "SELECT COUNT(*) as c FROM reports WHERE status != 'Closed' AND dueAt IS NOT NULL AND dueAt < ?", [now])[0]?.c ?? 0);
        const avgResolution = Number(allRows(db, "SELECT AVG(resolutionDays) as avg FROM reports WHERE status = 'Closed' AND resolutionDays IS NOT NULL")[0]?.avg ?? 0);
        return {
            total,
            open,
    ...
    async selectionExport(ids) {
        if (isPgAvailable()) {
            const result = await pgPool.query("SELECT * FROM reports WHERE id = ANY($1::text[]) ORDER BY date DESC", [ids]);
            return result.rows;
        }
        const db = await getDb();
        const placeholders = ids.map(() => "?").join(",");
        return allRows(db, `SELECT * FROM reports WHERE id IN (${placeholders}) ORDER BY date DESC`, ids);
    }
    ...
    async getLeaderboard(month, reporter) {
      ...
      `;
            ...
            const currentMonth = await pgPool.query(currentMonthSql, currentMonthParams);
            const availableMonths = await pgPool.query(`
        ...
        ORDER BY month DESC
      `);
            const availableReporters = await pgPool.query(`
      ...
      `, safeReporter ? [safeReporter] : []);
        ...
        const currentMonthRows = allRows(db, `SELECT
         CASE
           WHEN TRIM(COALESCE(reporter, '')) <> '' THEN TRIM(reporter)
           ELSE 'Unnamed reporter'
         END AS reporter,
       ...
       GROUP BY reporter
       ORDER BY report_count DESC, reporter ASC
       LIMIT 15`, safeReporter ? [currentMonthStart, currentMonthEnd, safeReporter] : [currentMonthStart, currentMonthEnd]);
        const availableMonthsRows = allRows(db, `SELECT DISTINCT strftime('%Y-%m', datetime(date, 'utc')) AS month
       FROM reports
       WHERE COALESCE(anonymous, 0) = 0
         AND LOWER(TRIM(COALESCE(reporter, ''))) <> 'anonymous'
       ORDER BY month DESC`);
        const availableReportersRows = allRows(db, `SELECT DISTINCT CASE
         WHEN TRIM(COALESCE(reporter, '')) <> '' THEN TRIM(reporter)
         ELSE 'Unnamed reporter'
       END AS reporter
       FROM reports
       ...
       ORDER BY rank ASC
       LIMIT 15`, safeMonth || safeReporter ? [safeMonth, safeReporter].filter(Boolean) : undefined);
        const monthlyByMonth = new Map();
        for (const row of monthlyRows) {
            if (Number(row.rank ?? 0) > 15)
                continue;
       ...
       GROUP BY reporter
       ORDER BY total_points DESC, total_reports DESC
       LIMIT 15`, safeReporter ? [safeReporter] : undefined);
        return {
