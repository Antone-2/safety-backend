...
export class HealthRepository {
    ...
    async findAll(filters) {
        ...
        return result.rows.map((row) => asHealthRecord(row));
    }
    async findById(id) {
        const result = await this.pool.query("SELECT * FROM health_surveillance WHERE id = $1", [id]);
        return result.rows[0] ? asHealthRecord(result.rows[0]) : null;
    }
    async create(data) {
        const result = await this.pool.query(`INSERT INTO health_surveillance (id, employee_id, employee_name, department, site, type, examination_date, next_due_date, frequency, results, findings, restrictions, fitness_for_work, doctor_name, doctor_registration, clinic_name, report_url, notes, created_by, created_at, updated_at)
       ...
       RETURNING *`, [
        ...
        return asHealthRecord(result.rows[0]);
    }
    ...
    async findExpiring(daysBefore) {
        const futureDate = new Date();
        futureDate.setDate(futureDate.getDate() + daysBefore);
        const result = await this.pool.query("SELECT * FROM health_surveillance WHERE next_due_date <= $1 AND fitness_for_work = TRUE ORDER BY next_due_date ASC", [futureDate.toISOString()]);
        return result.rows.map((row) => asHealthRecord(row));
    }
    async getStats() {
        ...
        const fitForWork = rows.filter((row) => row.fitness_for_work).reduce((sum, row) => sum + parseInt(row.count, 10), 0);
        const notFit = rows.filter((row) => !row.fitness_for_work).reduce((sum, row) => sum + parseInt(row.count, 10), 0);
        const audiometric = rows.filter((row) => row.type === "Audiometric").reduce((sum, row) => sum + parseInt(row.count, 10), 0);
        const respiratory = rows.filter((row) => row.type === "Respiratory").reduce((sum, row) => sum + parseInt(row.count, 10), 0);
