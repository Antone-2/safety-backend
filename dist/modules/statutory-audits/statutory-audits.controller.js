...
import { DeleteStatutoryAuditLocationSchema, UpsertStatutoryAuditRecordSchema, } from "./statutory-audits.types.js";
export function createStatutoryAuditController(service) {
    return {
        ...
        async upsertRecord(req, res) {
            ...
            await writeAuditLog({
                ...
                changes: beforeRecord
                    ? diffRecord(beforeRecord, record)
                    : [],
                actor: req.user,
                request: req,
            });
        ...
        async deleteLocation(req, res) {
            ...
            if (!deleted)
                throw new NotFoundError("Statutory audit location");
            await writeAuditLog({
...
export function createStatutoryAuditRouter() {
    ...
    router.post("/records", rbacMiddleware("compliance:update"), validate(UpsertStatutoryAuditRecordSchema), controller.upsertRecord);
    router.delete("/locations/:locationCategory/:locationName", rbacMiddleware("compliance:delete"), controller.deleteLocation);
    return router;
}
