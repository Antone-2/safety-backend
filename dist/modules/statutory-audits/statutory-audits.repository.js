...
function mapRow(row) {
    return {
        id: String(row.id),
        locationCategory: String(row.location_category),
...
export class StatutoryAuditRepository {
    ...
    async getMatrix(filters) {
        const records = await this.findAll(filters?.locationCategory || filters?.search
        ...
        const locations = Array.from(locationMap.values()).sort((a, b) => a.sortOrder - b.sortOrder || a.locationName.localeCompare(b.locationName));
        return {
            locations,
            auditTypes: ALL_AUDIT_TYPES,
        };
    }
    async upsertRecord(locationCategory, locationName, sortOrder, auditType, data) {
        await this.pool.query(`INSERT INTO statutory_audit_records (id, location_category, location_name, sort_order, audit_type, date_done, remarks, reference_no, created_at, updated_at)
    ...
    async existsByLocation(locationCategory, locationName) {
        const result = await this.pool.query(`SELECT 1
       FROM statutory_audit_records
       WHERE location_category = $1 AND location_name = $2
       LIMIT 1`, [locationCategory, locationName]);
        return (result.rowCount ?? 0) > 0;
    }
    async deleteByLocation(locationCategory, locationName) {
        await this.pool.query("DELETE FROM statutory_audit_records WHERE location_category = $1 AND location_name = $2", [locationCategory, locationName]);
    }
    async getSummary() {
