...
router.get("/users", authenticateUser, requireRole(["super-admin", "EHS-manager", "gm", "plant-manager", "factory-manager", "depot-admin"]), async (req, res) => {
    try {
        const filters = {};
        if (String(String(req.query.role)))
            filters.role = String(String(String(req.query.role)));
...
router.get("/users/:id", authenticateUser, requireRole(["super-admin", "EHS-manager"]), async (req, res) => {
    try {
...
router.get("/permissions", authenticateUser, requireRole(["super-admin", "EHS-manager"]), async (req, res) => {
    try {
...
router.post("/permissions", authenticateUser, requireRole(["super-admin", "EHS-manager"]), async (req, res) => {
    try {
...
router.get("/audit-logs", authenticateUser, requireRole(["super-admin", "EHS-manager"]), async (req, res) => {
    try {
        const filters = {};
        if (String(String(req.query.actor)))
            filters.actor = String(String(String(req.query.actor)));
        if (String(String(req.query.action)))
            filters.action = String(String(String(req.query.action)));
        if (String(String(req.query.resource)))
            filters.resource = String(String(String(req.query.resource)));
