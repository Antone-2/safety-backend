...
function tryWrite(clientId, client, payload) {
    try {
...
router.get("/", async (req, res) => {
    ...
    const rows = allRows(db, dataSql, params);
...
async function resolveAssignee(identifier) {
    if (!identifier)
        return null;
...
async function notifyAssignmentTarget(report, user, audienceLabel) {
    const db = await getDb();
...
router.patch("/:id/assign", async (req, res) => {
    ...
    const assignedByRaw = body.assignedBy || actorEmail || actorName;
    ...
    function addTarget(user, audience) {
        if (!user)
            return;
    ...
    if (supervisor)
        addTarget(supervisor, "the responsible supervisor (To)");
    for (const cc of copiedUsers)
        addTarget(cc, "a copied recipient (Cc)");
    if (assignedByRaw) {
        const assigner = await findUserByIdentifier(String(assignedByRaw));
        if (assigner)
            addTarget(assigner, "the administrator who assigned this task");
    }
