...
router.get("/carbon", authenticateUser, async (req, res) => {
    try {
        const filters = {};
        if (String(String(req.query.scope)))
            filters.scope = String(String(String(req.query.scope)));
        if (String(String(req.query.period)))
            filters.period = String(String(String(req.query.period)));
...
router.get("/energy", authenticateUser, async (req, res) => {
    try {
        const filters = {};
        if (String(String(req.query.source)))
            filters.source = String(String(String(req.query.source)));
        if (String(String(req.query.period)))
            filters.period = String(String(String(req.query.period)));
        if (String(String(req.query.site)))
            filters.site = String(String(String(req.query.site)));
