...
async function bootstrap() {
    try {
        ...
        startServer();
        const postgresBootstrapAttempts = Math.max(1, Math.min(10, Math.trunc(env.BOOTSTRAP_PG_MAX_ATTEMPTS ?? 5) || 5));
        const postgresAttemptTimeoutMs = Math.max(5_000, Math.min(120_000, Math.trunc(Number(process.env.BOOTSTRAP_PG_ATTEMPT_TIMEOUT_MS || 15_000)) || 15_000));
        let postgresReady = false;
        for (let attempt = 1; attempt <= postgresBootstrapAttempts; attempt++) {
            try {
                await withTimeout(runPostgresMigrations(), postgresAttemptTimeoutMs, `PostgreSQL bootstrap attempt ${attempt}`);
                postgresReady = true;
                logger.info("PostgreSQL migrations completed");
                break;
            }
            catch (error) {
                logger.warn({ err: error, attempt }, "PostgreSQL migration attempt failed");
        ...
        if (!postgresReady) {
            logger.error(`PostgreSQL is unavailable after ${postgresBootstrapAttempts} attempt${postgresBootstrapAttempts === 1 ? "" : "s"}; starting in degraded mode`);
