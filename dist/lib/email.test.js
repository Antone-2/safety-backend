...
import { buildIncidentNotification, buildReportAssignmentNotification, sendReportAssignmentNotifications, } from "./email.js";
const originalFetch = globalThis.fetch;
const originalBrevoKey = process.env.BREVO_API_KEY;
const originalSender = process.env.BREVO_SENDER_EMAIL;
afterEach(() => {
    globalThis.fetch = originalFetch;
    if (originalBrevoKey === undefined)
...
describe("email notifications", () => {
    it("buildIncidentNotification includes severity, location, and report details", () => {
        ...
        const result = buildIncidentNotification(report, "safety@example.com");
    ...
    it("buildReportAssignmentNotification creates assigner confirmation content", () => {
        ...
        const result = buildReportAssignmentNotification(report, { email: "assigner@example.com", role: "assigner" }, "assigner@example.com", "primary@example.com");
        ...
        expect(result.message).toMatch(/assigned report RPT-2002 to primary@example.com/i);
    });
    it("buildReportAssignmentNotification creates copied recipient content", () => {
        ...
        const result = buildReportAssignmentNotification(report, { email: "copy@example.com", role: "secondary" }, "assigner@example.com", "primary@example.com");
