...
export function pointsForSeverity(severity, weights = DEFAULT_SEVERITY_POINTS) {
...
function upsertReporterPoints(db, month, reporter, additionalPoints) {
    const now = new Date().toISOString();
    const row = db
        .prepare("SELECT reportCount, points FROM reporter_points WHERE month = ? AND reporter = ?")
        .getAsObject([month, reporter]);
    if (!row) {
        db
            .prepare("INSERT INTO reporter_points (month, reporter, reportCount, points, updatedAt) VALUES (?, ?, ?, ?, ?)")
            .run([month, reporter, 1, additionalPoints, now]);
        return;
    }
    db
        .prepare("UPDATE reporter_points SET reportCount = ?, points = ?, updatedAt = ? WHERE month = ? AND reporter = ?")
        .run([
...
export async function awardPointsForReport(db, report) {
...
export async function runMonthlyLeaderboardJob(db, options = {}) {
  ...
  `, [prevMonth, topN]);
    for (let i = 0; i < winners.length; i++) {
        ...
        const rank = i + 1;
        const existing = db
            .prepare("SELECT id FROM leaderboard_awards WHERE month = ? AND reporter = ? AND rank = ?")
            .getAsObject([prevMonth, w.reporter, rank]);
        if (existing?.id)
            continue;
        const awardId = `AWD-${Date.now()}-${Math.random().toString(16).slice(2)}`;
        const createdAt = new Date().toISOString();
        db
            .prepare("INSERT INTO leaderboard_awards (id, month, reporter, rank, reportCount, points, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)")
        ...
        db
            .prepare("INSERT INTO notifications (id, reportId, channel, recipient, subject, message, delivered, createdAt, read) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .run([
            notificationId,
...
export async function maybeRunMonthlyLeaderboardJob(db, settingsRow) {
