...
addMigration("046_auth_security", `
...
`);
addMigration("047_user_active", `ALTER TABLE users ADD COLUMN active INTEGER NOT NULL DEFAULT 1`);
addMigration("048_session_refresh", `ALTER TABLE auth_sessions ADD COLUMN refreshHash TEXT`);
addMigration("049_email_changes", `CREATE TABLE IF NOT EXISTS auth_email_changes (userId TEXT PRIMARY KEY, newEmail TEXT NOT NULL, codeHash TEXT NOT NULL, expiresAt TEXT NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, requestedAt TEXT NOT NULL)`);
addMigration("050_document_control_columns", `
...
`);
addMigration("051_document_control_tables", `
...
`);
addMigration("052_analytics_governance", `
...
`);
addMigration("053_ai_governance_guardrails", `
...
`);
addMigration("054_notification_center", `
...
`);
addMigration("055_operations_monitoring", `
...
`);
addMigration("056_security_hardening", `
...
addMigration("059_corrective_action_requests", `
...
`);
addMigration("060_corrective_action_request_note", `ALTER TABLE corrective_action_requests ADD COLUMN assigneeNote TEXT`);
addMigration("061_corrective_action_followup_fields", `
...
`);
addMigration("062_corrective_action_supervisor_comments", `ALTER TABLE corrective_action_requests ADD COLUMN supervisorComments TEXT DEFAULT '[]';`);
addMigration("063_corrective_action_acknowledgement_fields", `
...
export async function seedAdminUsers(db) {
    ...
    while (stmt.step()) {
        ...
        if (row?.email)
            existingEmails.add(String(row.email));
    }
    stmt.free();
    ...
    for (const user of toInsert) {
        ...
        if (!password || password.length < 12) {
            throw new Error(`Seed password for ${user.email} must be provided through environment and be at least 12 characters`);
        }
    }
    ...
    for (const u of toInsert) {
        ...
        db.prepare("INSERT OR IGNORE INTO users (id, email, passwordHash, name, role, createdAt) VALUES (?, ?, ?, ?, ?, ?)").run([
            uuidv4(),
            u.email,
            passwordHash,
            u.name,
    ...
    await saveDb(db);
    console.log(`Seeded missing default users: ${toInsert.map((u) => u.email).join(", ")}`);
}
...
export function addMigration(name, sql) {
