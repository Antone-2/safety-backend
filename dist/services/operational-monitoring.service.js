...
export class OperationalMonitoringService {
    async recordEvent(input) {
        const result = await pgPool.query(`INSERT INTO operational_events (id,type,source,status,message,metadata)
       VALUES ($1,$2,$3,$4,$5,$6::jsonb) RETURNING *`, [randomUUID(), input.type, input.source, input.status, input.message, JSON.stringify(input.metadata || {})]);
        return result.rows[0];
    }
    async recordSchedulerRun(input) {
        const result = await pgPool.query(`INSERT INTO scheduler_runs
       (id,job_name,status,started_at,finished_at,duration_ms,error,metadata)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb) RETURNING *`, [randomUUID(), input.jobName, input.status, input.startedAt, input.finishedAt || null,
            input.durationMs ?? null, input.error || null, JSON.stringify(input.metadata || {})]);
        return result.rows[0];
    }
    async recordSlowQuery(input) {
        const result = await pgPool.query(`INSERT INTO slow_query_logs (id,operation,duration_ms,threshold_ms,metadata)
       VALUES ($1,$2,$3,$4,$5::jsonb) RETURNING *`, [randomUUID(), input.operation, input.durationMs, input.thresholdMs, JSON.stringify(input.metadata || {})]);
        return result.rows[0];
    }
    async dashboard() {
