...
export class OperationalMonitoringService {
    ...
    async recordSchedulerRun(input) {
        ...
        db.prepare(`INSERT INTO scheduler_runs
    ...
    async recordSlowQuery(input) {
        ...
        db.prepare(`INSERT INTO slow_query_logs
    ...
    async dashboard() {
        const db = await getDb();
        const events = allRows(db, "SELECT type, status, COUNT(*) AS count FROM operational_events GROUP BY type, status");
        const scheduler = allRows(db, "SELECT jobName, status, COUNT(*) AS count, MAX(startedAt) AS lastStartedAt FROM scheduler_runs GROUP BY jobName, status");
        const slowRequests = allRows(db, "SELECT * FROM slow_query_logs ORDER BY createdAt DESC LIMIT 50").map((row) => ({ ...row, metadata: parseJson(row.metadata, {}) }));
        const recentEvents = allRows(db, "SELECT * FROM operational_events ORDER BY createdAt DESC LIMIT 50").map((row) => ({ ...row, metadata: parseJson(row.metadata, {}) }));
