...
export async function awardReporterPoints(input) {
    ...
    if (input.anonymous || !reporter || reporter.toLowerCase() === "anonymous")
        return;
    await pgPool.query(`INSERT INTO reporter_points (month, reporter, report_count, points, updated_at)
     VALUES ($1, $2, 1, $3, NOW())
...
export async function runMonthlyLeaderboard(now = new Date(), topN = 3) {
    ...
    const winners = await pgPool.query(`SELECT reporter, report_count, points
    ...
    for (const [index, winner] of winners.rows.entries()) {
        const rank = index + 1;
        const inserted = await pgPool.query(`INSERT INTO leaderboard_awards (month, reporter, rank, report_count, points)
       VALUES ($1, $2, $3, $4, $5)
       ...
       RETURNING id`, [month, winner.reporter, rank, winner.report_count, winner.points]);
        ...
        const user = await pgPool.query(`SELECT email FROM users
       WHERE active = TRUE AND (lower(email) = lower($1) OR lower(name) = lower($1))
       LIMIT 1`, [winner.reporter]);
        await pgPool.query(`INSERT INTO notification_jobs
