...
export class JsaService extends BaseService {
    ...
    async createJsa(data) {
        ...
        const result = await pgPool.query(`INSERT INTO jsa (id, title, description, location, department, status, steps, createdBy, createdAt, updatedAt, reviewedBy, reviewedAt)
    ...
    async getJsaById(id) {
        const result = await pgPool.query(`SELECT id, title, description, location, department, status, steps, createdBy AS "createdBy", createdAt AS "createdAt", updatedAt AS "updatedAt", reviewedBy AS "reviewedBy", reviewedAt AS "reviewedAt"
       FROM jsa WHERE id = $1`, [id]);
