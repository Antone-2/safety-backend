...
export async function runDatabaseMaintenance() {
    const [otps, sessions, rateLimits] = await Promise.all([
        ...
        pgPool.query("DELETE FROM auth_sessions WHERE expires_at < NOW() OR revoked_at < NOW() - INTERVAL '30 days'"),
        pgPool.query("DELETE FROM auth_rate_limits WHERE last_seen_at < NOW() - INTERVAL '7 days'"),
    ]);
    return {
