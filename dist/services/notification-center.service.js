...
export class NotificationCenterService {
    ...
    async processDue(limit = 25) {
        const result = await pgPool.query(`WITH due AS (
    ...
    async processJob(jobId) {
       ...
       last_error=$4,processing_started_at=NULL, updated_at=NOW() WHERE id=$5`, [status, attempts, nextAttemptAt, failed ? `${failed} recipient delivery failure(s)` : null, jobId]);
    ...
    async createDigest(input) {
       ...
       VALUES ($1,$2,$3,$4,$5::jsonb,TRUE,CASE WHEN $4='weekly' THEN NOW()+INTERVAL '7 days' ELSE NOW()+INTERVAL '1 day' END) RETURNING *`, [randomUUID(), input.userId || null, input.recipient, input.cadence || "daily", JSON.stringify(input.channels || ["email", "in-app"])]);
        const row = result.rows[0];
        return this.mapDigestRow(row);
    }
    async queueDigestItem(subscriptionId, input) {
      ...
      UPDATE notification_digest_subscriptions s SET processing_until=NOW()+INTERVAL '10 minutes',updated_at=NOW() FROM due WHERE s.id=due.id RETURNING s.*`, [Math.min(Math.max(limit, 1), 100)]);
        ...
        for (const subscription of subscriptions.rows) {
            ...
            if (items.rowCount) {
                ...
                const channels = (Array.isArray(subscription.channels) ? subscription.channels : ["email"]).filter((channel) => channel !== "in-app");
                if (channels.length)
                    await this.enqueue({ eventKey: "assignment.digest", workflow: "assignment", resourceType: "digest", resourceId: subscription.id, payload: { message: `${items.rowCount} assignment update${items.rowCount === 1 ? "" : "s"}:\n\n${messages.map((message, index) => `${index + 1}. ${message}`).join("\n")}`, count: items.rowCount }, recipients: channels.map((channel) => ({ channel, recipient: subscription.recipient })), createdBy: "Notification digest scheduler" });
        ...
        if (channel === "teams") {
            try {
                ...
                if (url.protocol !== "https:")
                    return { delivered: false, error: "Teams recipient must be an HTTPS workflow webhook URL." };
