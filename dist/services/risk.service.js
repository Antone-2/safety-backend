...
export class RiskService {
    ...
    async deleteRegister(id) {
        ...
        if (!existing)
            throw new Error("risk_registers not found");
    ...
    async getRiskDashboard() {
        ...
        const low = registers.filter((r) => r.riskLevel === "Low").length;
        const medium = registers.filter((r) => r.riskLevel === "Medium").length;
        const high = registers.filter((r) => r.riskLevel === "High").length;
        const critical = registers.filter((r) => r.riskLevel === "Critical").length;
