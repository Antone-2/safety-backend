...
export function addBusinessHours(start, hours, calendar = {}) {
    const workingDays = calendar.workingDays || [1, 2, 3, 4, 5];
    const startHour = calendar.startHour ?? 8;
    const endHour = calendar.endHour ?? 17;
    const timezone = calendar.timezone || "UTC";
    const initial = zonedParts(start, timezone);
    const holidays = new Set(calendar.holidays || []);
    const result = new Date(Date.UTC(initial.year, initial.month - 1, initial.day, initial.hour, initial.minute, initial.second));
    let remaining = hours;
    while (remaining > 0) {
        const dateKey = result.toISOString().slice(0, 10);
        const working = workingDays.includes(result.getUTCDay()) && !holidays.has(dateKey);
        if (!working || result.getUTCHours() >= endHour) {
            result.setUTCDate(result.getUTCDate() + 1);
            result.setUTCHours(startHour, 0, 0, 0);
            continue;
        }
        if (result.getUTCHours() < startHour)
            result.setUTCHours(startHour, 0, 0, 0);
        const available = endHour - result.getUTCHours();
        const consume = Math.min(remaining, available);
        result.setUTCHours(result.getUTCHours() + consume);
        remaining -= consume;
    }
...
function isQuietHour(start, end, timezone) {
    if (!start || !end)
        return false;
    ...
    const to = Number(end.slice(0, 2));
    return from < to ? hour >= from && hour < to : hour >= from || hour < to;
}
export class AssignmentSlaService {
    async applyPolicy(assignmentId) {
    ...
    async processDue(limit = 100) {
      ...
      UPDATE report_assignments a SET escalation_claimed_until=NOW()+INTERVAL '10 minutes' FROM due WHERE a.id=due.id RETURNING a.*`, [Math.min(Math.max(limit, 1), 500)]);
        ...
        for (const assignment of due.rows)
            results.push(await this.escalate(assignment));
        return results;
    }
    async escalate(assignment) {
        ...
        for (const recipient of unique.values())
            await this.notify(assignment, recipient, level.channels);
    ...
    async notify(assignment, recipient, requestedChannels) {
        ...
        if (!subscribedEvents.includes("escalated"))
            return;
        ...
        let channels = configured.filter((channel) => allowed.has(channel));
        if (quiet && !(critical && settings.critical_bypass_quiet_hours !== false))
        ...
        if (settings.digest_cadence && settings.digest_cadence !== "immediate" && !critical) {
            ...
            if (!subscription)
                subscription = await notificationCenterService.createDigest({ userId: settings.user_id, recipient: recipient.email, cadence: settings.digest_cadence, channels: channels.filter((channel) => channel !== "in-app") });
        ...
        const channelRecipients = channels.flatMap((channel) => {
            if (channel === "sms" || channel === "whatsapp")
                return settings.phone || settings.user_phone ? [{ channel: channel, recipient: settings.phone || settings.user_phone, name: recipient.name }] : [];
            if (channel === "teams")
                return settings.teams_recipient ? [{ channel: channel, recipient: settings.teams_recipient, name: recipient.name }] : [];
            return [{ channel: channel, recipient: recipient.email, name: recipient.name }];
        });
...
export function startAssignmentSlaScheduler() {
    const run = async () => {
        try {
            ...
            console.error("Assignment SLA or notification processing failed", error);
        }
    };
