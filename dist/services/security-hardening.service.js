...
export class SecurityHardeningService {
    async upsertPolicy(policyKey, policyValue, description, actor) {
        ...
        db.prepare(`INSERT OR REPLACE INTO security_policies
    ...
    async listPolicies() {
        const db = await getDb();
        return allRows(db, "SELECT * FROM security_policies ORDER BY policyKey ASC").map((row) => ({ ...row, policyValue: parseJson(row.policyValue, {}) }));
    }
    async recordFileScan(input) {
        const findings = [];
        const allowedTypes = (process.env.ALLOWED_UPLOAD_MIME_TYPES ||
        ...
        if (/\.(exe|bat|cmd|ps1|js|vbs|scr|dll)$/i.test(input.fileName || input.fileKey)) {
        ...
        db.prepare(`INSERT INTO file_security_scans
       (id, fileKey, fileName, mimeType, sizeBytes, checksum, status, scanner, findings, scannedAt, uploadedBy, createdAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(Object.values(row));
        await saveDb(db);
        return { ...row, findings };
    }
    async listFileScans(status) {
        const db = await getDb();
        const rows = status
            ? allRows(db, "SELECT * FROM file_security_scans WHERE status = ? ORDER BY createdAt DESC LIMIT 250", [status])
            : allRows(db, "SELECT * FROM file_security_scans ORDER BY createdAt DESC LIMIT 250");
        return rows.map((row) => ({
            ...row,
            findings: parseJson(row.findings, []),
        }));
    }
    async upsertRetentionPolicy(data, actor) {
        const db = await getDb();
        const updatedAt = now();
        const row = {
            id: data.id || uuidv4(),
        ...
        db.prepare(`INSERT OR REPLACE INTO data_retention_policies
    ...
    async listRetentionPolicies() {
        const db = await getDb();
        return allRows(db, "SELECT * FROM data_retention_policies ORDER BY resourceType ASC").map((row) => ({ ...row, legalHold: Boolean(row.legalHold) }));
    }
    async recordSecretRotation(data, actor) {
        const lastRotatedAt = data.lastRotatedAt || now();
        const frequency = Number(data.rotationFrequencyDays || 90);
        const nextRotationDueAt = data.nextRotationDueAt ||
        ...
        db.prepare(`INSERT OR REPLACE INTO secrets_rotation_log
    ...
    async listSecretRotations() {
        const db = await getDb();
        return allRows(db, "SELECT * FROM secrets_rotation_log ORDER BY nextRotationDueAt ASC");
    }
    async dashboard() {
        ...
        return {
            policies,
            retention,
            secretRotationsDue: rotations.filter((item) => item.status !== "completed" &&
            ...
            owaspChecklist: [
                ...
                },
                {
                    ...
                    status: "partial",
                    evidence: "JWT/OTP hashing, optional backup encryption, secrets rotation log",
                },
                {
                    item: "Injection",
                    status: "covered",
