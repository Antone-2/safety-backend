...
export async function createClosureRequest(input) {
    ...
    await pgPool.query(`INSERT INTO report_closure_requests
     (id, report_id, submitted_by, submitted_by_email, resolution_notes, photos, status, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6::text[], 'pending', $7, $8)`, [id, input.reportId, input.submittedBy, input.submittedByEmail, input.resolutionNotes, input.photos, now, now]);
    await writeAuditLog({
    ...
    const reportResult = await pgPool.query("SELECT id, location, description, severity, status, assigned_to, assigned_to_copy FROM reports WHERE id = $1", [input.reportId]);
    const report = reportResult.rows[0];
    if (report) {
        const recipients = [
            { channel: "email", recipient: input.submittedByEmail, name: input.submittedBy },
        ];
        const assignedTo = String(report.assigned_to ?? "");
        const assignedToCopy = Array.isArray(report.assigned_to_copy)
            ? report.assigned_to_copy.map((v) => String(v))
            : [];
        for (const email of assignedToCopy) {
            if (email && email !== input.submittedByEmail) {
                recipients.push({ channel: "email", recipient: email, name: email });
            }
        }
...
export async function approveClosureRequest(input) {
    const now = new Date().toISOString();
    const requestResult = await pgPool.query("SELECT * FROM report_closure_requests WHERE id = $1 AND report_id = $2 AND status = 'pending'", [input.requestId, input.reportId]);
    ...
    try {
        ...
        await client.query(`UPDATE report_closure_requests
       SET status = 'approved', reviewed_by = $1, reviewed_by_email = $2,
           review_notes = $3, updated_at = $4
       WHERE id = $5`, [input.reviewedBy, input.reviewedByEmail, input.reviewNotes || null, now, input.requestId]);
        await client.query("UPDATE reports SET status = 'Closed', updated_at = $1 WHERE id = $2", [now, input.reportId]);
    ...
    if (updatedRequest) {
        await notificationCenterService.enqueue({
            ...
            recipients: [
                { channel: "email", recipient: updatedRequest.submittedByEmail, name: updatedRequest.submittedBy },
            ],
...
export async function rejectClosureRequest(input) {
    ...
    await pgPool.query(`UPDATE report_closure_requests
     SET status = 'rejected', reviewed_by = $1, reviewed_by_email = $2,
         review_notes = $3, updated_at = $4
     WHERE id = $5`, [input.reviewedBy, input.reviewedByEmail, input.reviewNotes, now, input.requestId]);
    await writeAuditLog({
...
export async function getClosureRequest(requestId, reportId) {
    const result = await pgPool.query("SELECT * FROM report_closure_requests WHERE id = $1 AND report_id = $2", [requestId, reportId]);
    const row = result.rows[0];
    if (!row)
        return null;
    return mapClosureRequest(row);
}
export async function listClosureRequests(reportId) {
...
function mapClosureRequest(row) {
    const photos = Array.isArray(row.photos)
        ? row.photos.map((photo) => String(photo))
