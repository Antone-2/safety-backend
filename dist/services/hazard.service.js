...
export class HazardService {
    ...
    async createReport(data) {
        ...
        const timestamp = now();
        const result = await pgPool.query(`INSERT INTO hazard_reports (
    ...
    async update(id, data) {
        ...
        params.push(id);
        const result = await pgPool.query(`UPDATE hazard_reports SET ${fields.join(", ")} WHERE id = $${params.length} RETURNING *`, params);
        return result.rows[0] ? mapRow(result.rows[0]) : null;
    }
    async delete(id) {
