...
export class WorkflowEngine {
  ...
  canTransition(
    ...
    if (!transition.requiredPermission) return true;
    return (
  ...
  transition(
    ...
    if (!transition) {
      throw new BusinessRuleError(
        `Invalid workflow transition: ${currentState} -> ${event}`,
      );
    }

    if (!this.canTransition(currentState, event, actor)) {
      throw new BusinessRuleError(
        `Actor cannot perform workflow transition: ${event}`,
      );
    }
  ...
  private findTransition(currentState: string, event: string) {
    return this.definition.transitions.find(
      (transition) =>
        transition.from === currentState && transition.event === event,
    );
  }
}

export const INCIDENT_WORKFLOW: WorkflowDefinition = {
  name: "incident",
  initialState: "Open",
  finalStates: ["Closed"],
  transitions: [
...
export const REPORT_WORKFLOW: WorkflowDefinition = {
  name: "report",
  initialState: "Open",
  finalStates: ["Closed"],
  transitions: [
