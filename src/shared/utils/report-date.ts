...
function parseLocalSlashDateParts(
  year: number,
  month: number,
  day: number,
  hour: number,
...
function parseLocalSlashDate(value: string, referenceDate: Date = new Date()): string | undefined {
  ...
  const buildCandidate = (monthFirst: boolean) =>
    parseLocalSlashDateParts(
      year,
  ...
  const preferredMonthFirst =
    isUnambiguouslyMonthFirst || (!isUnambiguouslyDayFirst && dateOrder === "mdy");
...
export function tryParseReportDate(value: unknown, referenceDate: Date = new Date()): string | undefined {
...
export function parseReportDate(value: unknown, referenceDate: Date = new Date()): string {
