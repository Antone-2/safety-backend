...
export interface ImageSet {
  original: { key: string; url: string; width: number; height: number };
  thumbnails: { size: number; key: string; url: string }[];
}

export class ImageProcessor {
  async process(key: string, buffer: Buffer): Promise<ImageSet> {
    ...
    const thumbnails = await Promise.all(
      thumbSizes.map(async (size: number) => {
    ...
    return {
      ...
      thumbnails: thumbnails.map((t: { size: number; buffer: Buffer }) => ({
        size: t.size,
