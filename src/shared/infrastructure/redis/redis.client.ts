...
export async function ensureRedisProductionReady() {
  ...
  if (isLocalhostRedisUrl(env.REDIS_URL)) {
    throw new RedisRequiredError(
      "REDIS_URL must not point to localhost when REQUIRE_REDIS=true in this environment.",
    );
  }
  ...
  if (!bullMqReady) {
    throw new RedisVersionUnsupportedError(
      `Redis ${bullMqRedisVersion || "unknown"} detected. BullMQ requires Redis ${MINIMUM_BULLMQ_REDIS_MAJOR}.0.0 or newer.`,
    );
  }
