...
function asEquipment(row: Record<string, unknown>): Equipment {
  return {
    id: String(row.id),
    name: String(row.name),
    type: String(row.type) as Equipment["type"],
...
function asInspection(row: Record<string, unknown>): EquipmentInspection {
  return {
    id: String(row.id),
    equipmentId: String(row.equipment_id),
    inspector: String(row.inspector),
    inspectionDate: String(row.inspection_date),
    inspectionType: String(row.inspection_type) as EquipmentInspection["inspectionType"],
...
export class EquipmentRepository {
  ...
  async findAll(filters?: Record<string, unknown>) {
    ...
    return result.rows.map((row) => asEquipment(row as unknown as Record<string, unknown>));
  }

  async findById(id: string) {
    const result = await this.pool.query("SELECT * FROM equipment WHERE id = $1", [id]);
    return result.rows[0] ? asEquipment(result.rows[0] as unknown as Record<string, unknown>) : null;
  }

  async create(data: CreateEquipmentInput) {
    const result = await this.pool.query(
      `INSERT INTO equipment (id, name, type, category, asset_tag, serial_number, manufacturer, model, location, site, department, purchase_date, installation_date, warranty_expiry, last_inspection_date, next_inspection_date, inspection_frequency, status, "condition", assigned_to, notes, photo_url, created_by, created_at, updated_at)
    ...
    return asEquipment(result.rows[0] as unknown as Record<string, unknown>);
  }
  ...
  async createInspection(data: CreateEquipmentInspectionInput) {
    ...
    const inspection = asInspection(result.rows[0] as unknown as Record<string, unknown>);
