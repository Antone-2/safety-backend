...
function mapUser(row: Record<string, unknown>): ManagedUser {
  ...
  return {
    ...
    role: String(row.role) as ManagedUser["role"],
...
export class UsersRepository {
  ...
  async findAll(filters: UserFilters = {}) {
    ...
    if (filters.search) {
      add(
        `(u.name ILIKE ? OR u.email ILIKE ? OR COALESCE(u.site, '') ILIKE ? OR COALESCE(u.department, '') ILIKE ? OR COALESCE(u.employee_no, '') ILIKE ?)`,
        `%${filters.search}%`,
      );
    ...
    if (filters.role) add(`u.role = ?`, filters.role);
    if (filters.site) add(`u.site = ?`, filters.site);
    if (filters.department) add(`u.department = ?`, filters.department);
    if (typeof filters.active === "boolean") add(`u.active = ?`, filters.active);
    if (filters.lineManagerId) add(`u.line_manager_id = ?`, filters.lineManagerId);
    if (filters.supervisorId) add(`u.supervisor_id = ?`, filters.supervisorId);

    const result = await this.pool.query(
      `${this.baseSelect()} ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY u.created_at DESC`,
      params,
    );
    return result.rows.map((row) => mapUser(row as unknown as Record<string, unknown>));
  }

  async findById(id: string) {
    const result = await this.pool.query(
      `${this.baseSelect()} WHERE u.id = $1 LIMIT 1`,
      [id],
    );
    return result.rows[0]
      ? mapUser(result.rows[0] as unknown as Record<string, unknown>)
      : null;
  }

  async findByEmail(email: string) {
    const result = await this.pool.query(
      `${this.baseSelect()} WHERE lower(u.email) = $1 LIMIT 1`,
  ...
  async update(id: string, input: UpdateUserInput) {
    ...
    if (input.email !== undefined) add("email", normalizeEmail(input.email));
    if (input.name !== undefined) add("name", input.name);
    if (input.role !== undefined) add("role", input.role);
    if (input.phone !== undefined) add("phone", input.phone ?? null);
    if (input.site !== undefined) add("site", input.site ?? null);
    if (input.department !== undefined) add("department", input.department ?? null);
    if (input.employeeNo !== undefined) add("employee_no", input.employeeNo ?? null);
    if (input.jobTitle !== undefined) add("job_title", input.jobTitle ?? null);
    if (input.lineManagerId !== undefined) add("line_manager_id", input.lineManagerId ?? null);
    if (input.supervisorId !== undefined) add("supervisor_id", input.supervisorId ?? null);
  ...
  async setDelegation(id: string, input: DelegationInput) {
    await this.pool.query(
      `UPDATE users
       SET delegated_to_user_id = $1,
           ...
           updated_at = NOW()
       WHERE id = $4`,
      [input.delegatedToUserId, input.delegatedFrom, input.delegatedUntil, id],
    );
    return this.findById(id);
  }

  async clearDelegation(id: string) {
    await this.pool.query(
      `UPDATE users
