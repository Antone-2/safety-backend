...
function mapRow(row: Record<string, unknown>): WibaClaim {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as WibaClaim["status"],
    stage: String(row.stage ?? "None"),
    amountAwardedKes:
      row.amount_awarded_kes == null ? undefined : Number(row.amount_awarded_kes),
    companyClaimKes:
      row.company_claim_kes == null ? undefined : Number(row.company_claim_kes),
    outstandingDocuments: Array.isArray(row.outstanding_documents)
      ? (row.outstanding_documents as string[])
      : [],
...
export class WibaRepository {
  ...
  async seedDefaultsIfEmpty(): Promise<void> {
    const countResult = await this.pool.query(
      "SELECT COUNT(*)::int AS count FROM wiba_claims",
    );
  ...
  async findAll(): Promise<WibaClaim[]> {
  ...
  async update(id: string, data: WibaClaimPatch): Promise<WibaClaim | null> {
    const fields: string[] = [];
    ...
    if (data.claimNo !== undefined) push("claim_no", data.claimNo);
    if (data.dateOfInjury !== undefined) push("date_of_injury", data.dateOfInjury);
    if (data.natureOfInjury !== undefined) push("nature_of_injury", data.natureOfInjury);
    if (data.claimantName !== undefined) push("claimant_name", data.claimantName);
    if (data.status !== undefined) push("status", data.status);
    if (data.stage !== undefined) push("stage", data.stage);
    if (data.amountAwardedKes !== undefined) push("amount_awarded_kes", data.amountAwardedKes);
    if (data.companyClaimKes !== undefined) push("company_claim_kes", data.companyClaimKes);
    ...
    if (fields.length === 0) {
      ...
      return current.rows[0] ? mapRow(current.rows[0] as Record<string, unknown>) : null;
    }
