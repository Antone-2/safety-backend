...
function asHeightWork(row: Record<string, unknown>): HeightWork {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as HeightWork["status"],
...
export class HeightWorkRepository {
  ...
  async findAll(filters?: Record<string, unknown>) {
    ...
    return result.rows.map((row) => asHeightWork(row as unknown as Record<string, unknown>));
  }

  async findById(id: string) {
    const result = await this.pool.query("SELECT * FROM height_works WHERE id = $1", [id]);
    return result.rows[0] ? asHeightWork(result.rows[0] as unknown as Record<string, unknown>) : null;
  }

  async create(data: CreateHeightWorkInput) {
    const permitNo = `HGT-${new Date().getFullYear()}-${String(Date.now()).slice(-4)}`;
    const result = await this.pool.query(
      `INSERT INTO height_works (id, permit_no, location, building, floor, task_description, height, fall_protection, rescue_plan, harness_inspection_date, anchor_point_inspected, workers_count, workers, supervisor, start_date, end_date, status, incident_report, photos, notes, created_by, created_at, updated_at)
    ...
    return asHeightWork(result.rows[0] as unknown as Record<string, unknown>);
  }
