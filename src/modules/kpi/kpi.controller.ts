...
export function createKpiController(service: KpiService) {
  return {
    async getDefinitions(req: AuthRequest, res: Response) {
    ...
    async getDefinition(req: AuthRequest, res: Response) {
      const definition = await service.getDefinitionById(String(req.params.id));
      if (!definition) throw new NotFoundError("KPI definition");
      res.json({ data: definition });
    },

    async createDefinition(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateKpiDefinitionSchema>;
    ...
    async updateDefinition(req: AuthRequest, res: Response) {
      const before = await service.getDefinitionById(String(req.params.id));
      if (!before) throw new NotFoundError("KPI definition");
      const definition = await service.updateDefinition(String(req.params.id), req.body);
      await writeAuditLog({
        action: "kpi.definition.updated",
        resourceType: "kpi_definition",
        resourceId: String(req.params.id),
        changes: diffRecord(before as unknown as Record<string, unknown>, definition as unknown as Record<string, unknown>),
        actor: req.user,
        request: req,
      });
      res.json({ data: definition });
    ...
    async deleteDefinition(req: AuthRequest, res: Response) {
      const deleted = await service.deleteDefinition(String(req.params.id));
      if (!deleted) throw new NotFoundError("KPI definition");
      await writeAuditLog({
    ...
    async getValues(req: AuthRequest, res: Response) {
      const filters: { definitionId?: string; periodStart?: string; periodEnd?: string } = {};
      const { definitionId, periodStart, periodEnd } = req.query;
      if (definitionId) filters.definitionId = String(definitionId);
      if (periodStart) filters.periodStart = String(periodStart);
      if (periodEnd) filters.periodEnd = String(periodEnd);
      const values = await service.getValues(filters);
      res.json({ data: values });
    },

    async getValue(req: AuthRequest, res: Response) {
      const value = await service.getValueById(String(req.params.id));
      if (!value) throw new NotFoundError("KPI value");
      res.json({ data: value });
    },

    async createValue(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateKpiValueSchema>;
    ...
    async updateValue(req: AuthRequest, res: Response) {
      ...
      if (!before) throw new NotFoundError("KPI value");
      const value = await service.updateValue(String(req.params.id), req.body);
      await writeAuditLog({
        action: "kpi.value.updated",
        resourceType: "kpi_value",
        resourceId: String(req.params.id),
        changes: diffRecord(before as unknown as Record<string, unknown>, value as unknown as Record<string, unknown>),
        actor: req.user,
        request: req,
      });
      res.json({ data: value });
    ...
    async deleteValue(req: AuthRequest, res: Response) {
      ...
      if (!deleted) throw new NotFoundError("KPI value");
      await writeAuditLog({
