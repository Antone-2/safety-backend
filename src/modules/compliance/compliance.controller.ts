...
import {
  ...
  UpdateComplianceObligationSchema,
...
export function createComplianceController(service: ComplianceService) {
  return {
    async getObligations(req: AuthRequest, res: Response) {
      ...
      if (responsibility) filters.responsibility = String(responsibility);
    ...
    async getObligationById(req: AuthRequest, res: Response) {
      ...
      if (!obligation) throw new NotFoundError("Compliance obligation");
      res.json({ data: obligation });
    },

    async createObligation(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateComplianceObligationSchema>;
    ...
    async updateObligation(req: AuthRequest, res: Response) {
      ...
      if (!before) throw new NotFoundError("Compliance obligation");
    ...
    async deleteObligation(req: AuthRequest, res: Response) {
      ...
      if (!deleted) throw new NotFoundError("Compliance obligation");
      await writeAuditLog({
    ...
    async getAudits(req: AuthRequest, res: Response) {
      const filters: Record<string, unknown> = {};
    ...
    async getAuditById(req: AuthRequest, res: Response) {
      const audit = await service.getAuditById(String(req.params.id));
      if (!audit) throw new NotFoundError("Compliance audit");
      res.json({ data: audit });
    },

    async createAudit(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateComplianceAuditSchema>;
    ...
    async updateAudit(req: AuthRequest, res: Response) {
      const before = await service.getAuditById(String(req.params.id));
      if (!before) throw new NotFoundError("Compliance audit");
      const audit = await service.updateAudit(String(req.params.id), req.body);
      await writeAuditLog({
        action: "compliance.audit.updated",
        resourceType: "compliance_audit",
        resourceId: String(req.params.id),
        changes: diffRecord(before as unknown as Record<string, unknown>, audit as unknown as Record<string, unknown>),
        actor: req.user,
        request: req,
      });
      res.json({ data: audit });
    ...
    async deleteAudit(req: AuthRequest, res: Response) {
      const deleted = await service.deleteAudit(String(req.params.id));
      if (!deleted) throw new NotFoundError("Compliance audit");
      await writeAuditLog({
    ...
    async getLegalUpdates(req: AuthRequest, res: Response) {
      const filters: Record<string, unknown> = {};
    ...
    async getLegalUpdateById(req: AuthRequest, res: Response) {
      const update = await service.getLegalUpdateById(String(req.params.id));
      if (!update) throw new NotFoundError("Legal update");
      res.json({ data: update });
    },

    async createLegalUpdate(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateLegalUpdateSchema>;
    ...
    async updateLegalUpdate(req: AuthRequest, res: Response) {
      const before = await service.getLegalUpdateById(String(req.params.id));
      if (!before) throw new NotFoundError("Legal update");
      const legalUpdate = await service.updateLegalUpdate(String(req.params.id), req.body);
      await writeAuditLog({
        action: "compliance.legal_update.updated",
        resourceType: "legal_update",
        resourceId: String(req.params.id),
        changes: diffRecord(before as unknown as Record<string, unknown>, legalUpdate as unknown as Record<string, unknown>),
        actor: req.user,
        request: req,
      });
    ...
    async deleteLegalUpdate(req: AuthRequest, res: Response) {
      const deleted = await service.deleteLegalUpdate(String(req.params.id));
      if (!deleted) throw new NotFoundError("Legal update");
      await writeAuditLog({
