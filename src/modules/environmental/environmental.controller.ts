...
export function createEnvironmentalController(service: EnvironmentalService) {
  return {
    async getWaste(req: AuthRequest, res: Response) {
    ...
    async createWaste(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateWasteSchema>;
    ...
    async updateWaste(req: AuthRequest, res: Response) {
      const before = (await service.getWaste({ id: String(req.params.id) }))[0];
      if (!before) throw new NotFoundError("Waste record");
    ...
    async createEmission(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateEmissionSchema>;
    ...
    async updateEmission(req: AuthRequest, res: Response) {
      const before = (
        await service.getEmissions({ id: String(req.params.id) })
      )[0];
      if (!before) throw new NotFoundError("Emission record");
      const emission = await service.updateEmission(
      ...
      await writeAuditLog({
        ...
        changes: diffRecord(
          before as unknown as Record<string, unknown>,
          emission as unknown as Record<string, unknown>,
        ),
        actor: req.user,
        request: req,
      });
    ...
    async createChemical(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateChemicalSchema>;
    ...
    async updateChemical(req: AuthRequest, res: Response) {
      const before = (
        await service.getChemicals({ id: String(req.params.id) })
      )[0];
      if (!before) throw new NotFoundError("Chemical");
      const chemical = await service.updateChemical(
        String(req.params.id),
        req.body,
      );
      await writeAuditLog({
        ...
        changes: diffRecord(
          before as unknown as Record<string, unknown>,
          chemical as unknown as Record<string, unknown>,
        ),
        actor: req.user,
        request: req,
      });
    ...
    async createSpill(req: AuthRequest, res: Response) {
      const data = req.body as z.infer<typeof CreateSpillSchema>;
    ...
    async updateSpill(req: AuthRequest, res: Response) {
      const before = (
        await service.getSpills({ id: String(req.params.id) })
      )[0];
      if (!before) throw new NotFoundError("Spill record");
      ...
      await writeAuditLog({
        ...
        changes: diffRecord(
          before as unknown as Record<string, unknown>,
          spill as unknown as Record<string, unknown>,
        ),
        actor: req.user,
        request: req,
      });
...
export function createEnvironmentalRouter() {
  ...
  router.get(
    "/waste",
    rbacMiddleware("environmental:read"),
    controller.getWaste,
  );
  router.post(
    "/waste",
    rbacMiddleware("environmental:create"),
  ...
  router.get(
    "/emissions",
    rbacMiddleware("environmental:read"),
    controller.getEmissions,
  );
  router.post(
    "/emissions",
    rbacMiddleware("environmental:create"),
    validate(CreateEmissionSchema),
    controller.createEmission,
  );
  router.patch(
    "/emissions/:id",
    rbacMiddleware("environmental:update"),
  ...
  router.get(
    "/chemicals",
    rbacMiddleware("environmental:read"),
    controller.getChemicals,
  );
  router.post(
    "/chemicals",
    rbacMiddleware("environmental:create"),
    validate(CreateChemicalSchema),
    controller.createChemical,
  );
  router.patch(
    "/chemicals/:id",
    rbacMiddleware("environmental:update"),
  ...
  router.get(
    "/spills",
    rbacMiddleware("environmental:read"),
    controller.getSpills,
  );
  router.post(
    "/spills",
    rbacMiddleware("environmental:create"),
    validate(CreateSpillSchema),
    controller.createSpill,
  );
  router.patch(
    "/spills/:id",
    rbacMiddleware("environmental:update"),
    validate(UpdateSpillSchema),
    controller.updateSpill,
  );
