...
function asRecord(row: Record<string, unknown>): VisitorRecord {
  return {
    id: String(row.id),
    ...
    accessStatus: String(row.access_status) as VisitorRecord["accessStatus"],
    inductionStatus: String(row.induction_status) as VisitorRecord["inductionStatus"],
...
export class VisitorsRepository {
  ...
  async findAll(filters?: Record<string, unknown>) {
    ...
    const result = await this.pool.query(
      `SELECT * FROM visitor_access_records ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY visit_date DESC, created_at DESC`,
      params,
    );
  ...
  async create(data: CreateVisitorInput) {
    ...
    const result = await this.pool.query(
      `INSERT INTO visitor_access_records (
  ...
  async update(id: string, data: UpdateVisitorInput) {
    ...
    const result = await this.pool.query(
      `UPDATE visitor_access_records SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`,
      params,
    );
    return result.rows[0] ? asRecord(result.rows[0] as Record<string, unknown>) : null;
  }
  ...
  async findOnSite() {
    const result = await this.pool.query(
      `SELECT * FROM visitor_access_records
