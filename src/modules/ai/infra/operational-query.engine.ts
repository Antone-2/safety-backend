...
import type {
  AiGuardrailSettings,
  AiQueryDomain,
  BaseQueryExecutionInput,
  DomainQueryHandler,
  Json,
  NonReportAiQueryDomain,
  QueryEngineDeps,
} from "./query-domain.contract.js";
...
export function detectAiQueryDomain(query: string): AiQueryDomain {
  const text = query.toLowerCase();
  if (
    /permit|ptw|permit to work|hot work|confined space|electrical permit|excavation permit|height work permit|approval stage|permit expiry|expired permit/.test(
      text,
    )
  ) {
    return "permits";
  ...
  if (
    /capa|corrective action|preventive action|action plan|follow-up action|follow up action|overdue action|closure action/.test(
      text,
    )
  ) {
    return "capa";
  ...
  if (
    /training|course|certificate|competency|trainer|attendance|expiry|expired training|matrix/.test(
      text,
    )
  ) {
    return "training";
...
function compactTrainingRecord(record: Record<string, unknown>) {
  return {
    id: String(record.id ?? ""),
...
export class OperationalQueryEngine {
  ...
  private permits = new PermitsService(new PermitsRepository(pgPool));
  private handlers: Record<NonReportAiQueryDomain, DomainQueryHandler>;
  ...
  async executeDomainQuery(domain: NonReportAiQueryDomain, input: BaseQueryExecutionInput): Promise<Json> {
    return this.handlers[domain].execute(input);
  }

  private async executeTrainingQuery(input: BaseQueryExecutionInput): Promise<Json> {
    const text = input.governedInput.query.toLowerCase();
    const trainingFilters: Record<string, unknown> = {};
    ...
    const scopedRecords = (records as Array<Record<string, unknown>>).slice(
      0,
      input.governedInput.maxSourceRecords,
    );
    ...
    const assistantAnswer = await this.deps.llm.generate(
      "You are Crown Safety Training AI. Answer only from the trusted training context provided. Do not invent employee completions, expiries, or counts. Cite training record IDs inline when making factual claims.",
      [
    ...
    await this.deps.savePrediction(
      "ai-query",
      input.governedInput,
      output,
      Number(output.metadata.confidence),
      input.user?.id,
    );
  ...
  private async executeCapaQuery(input: BaseQueryExecutionInput): Promise<Json> {
    const text = input.governedInput.query.toLowerCase();
    ...
    const [summaryResult, recordsResult] = await Promise.all([
      ...
      pgPool.query(
        `SELECT id, capa_no, title, type, status, priority, owner, department, site, due_date, source, action_plan
         FROM capa
         WHERE ${whereSql}
         ORDER BY due_date ASC NULLS LAST, updated_at DESC
         LIMIT $${idx}`,
        [...params, input.governedInput.maxSourceRecords],
      ),
    ]);
    ...
    const assistantAnswer = await this.deps.llm.generate(
      "You are Crown Safety CAPA AI. Answer only from the trusted CAPA context provided. Do not invent actions, owners, due dates, or closure states. Cite CAPA IDs inline when making factual claims.",
      [
        `User question: ${input.governedInput.query}`,
  ...
  private async executePermitsQuery(input: BaseQueryExecutionInput): Promise<Json> {
    const text = input.governedInput.query.toLowerCase();
    ...
    const matchingPermits = (allPermits as Array<Record<string, unknown>>)
      .filter((permit) => {
        if (/expired/.test(text)) {
    ...
    const summary = {
      ...
      expired: matchingPermits.filter(
        (permit) =>
          permit.status === "expired" || String(permit.endDate ?? "") < nowIso,
      ).length,
    ...
    const assistantAnswer = await this.deps.llm.generate(
      "You are Crown Safety Permit AI. Answer only from the trusted permit context provided. Do not invent permit status, dates, locations, or approvers. Cite permit IDs inline when making factual claims.",
      [
        `User question: ${input.governedInput.query}`,
    ...
    const output = {
      ...
      metadata: {
        ...
        confidenceLevel:
          trustedContext.matchingPermits.length > 0 ? "high" : "medium",
        modelVersion: this.deps.model,
        processingTimeMs: Date.now() - input.startedAt,
        sources: trustedContext.matchingPermits.map((permit) => permit.id).filter(Boolean),
        warnings: [
