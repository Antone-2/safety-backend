...
export class ReportQueryEngine {
  ...
  private async getReportSummary(filters: ReportFilters, fallbackReports: ReportRow[] = []) {
    const maybeReports = this.reports as ReportsService & {
      summary?: (filters: ReportFilters) => Promise<SummarySnapshot>;
    };
    ...
    const criticalOpen = reports.filter(
      (report) => report.severity === "Critical" && report.status !== "Closed",
    ).length;
    const overdue = reports.filter(isOverdue).length;
    const recordableIncidents = reports.filter(isRecordable).length;
    const nearMissCount = reports.filter(
      (report) => report.isNearMiss || /near miss|near-miss/i.test(`${report.description} ${report.category}`),
    ).length;
  ...
  private async getTopReporters(limit: number, fallbackReports: ReportRow[] = []) {
    const maybeReports = this.reports as ReportsService & {
      topReportersMonthToDate?: (limit: number) => Promise<Array<{ reporter: string; reportCount: number }>>;
    };
    ...
    return countBy(
      fallbackReports
        ...
        .filter((reporter): reporter is string => Boolean(reporter)),
    ).slice(0, limit).map(([reporter, reportCount]) => ({ reporter, reportCount }));
  }
  ...
  private async generateEvidenceBoundAnswer(input: {
    ...
    const answer = await this.deps.llm.generate(
      "You are Crown Safety Data AI. Answer only from the trusted analytics context you are given. Do not invent KPIs, counts, dates, locations, departments, or incidents. If the context is insufficient, say exactly what is missing. Use a concise executive tone. Cite report IDs inline when making factual claims.",
      [
  ...
  async executeReportQuery(input: BaseQueryExecutionInput): Promise<Json> {
    ...
    const reportsData = initialResult.data as ReportRow[];
    ...
    if (total === 0) {
      ...
      const output = {
        ...
        data: {
          ...report,
          export:
            input.governedInput.exportFormat === "html"
    ...
    const tableCitations = Object.fromEntries(
      selectedTables.map((table) => {
        if (table.title.includes("Monthly")) return [table.title, sourceRecords];
        ...
        if (table.title.includes("Severity")) return [table.title, sourceRecords];
    ...
    const sqlFirst = await this.generateEvidenceBoundAnswer({
      ...
      summary: {
        ...
        severityCounts:
          typeof summary?.severityCounts === "object" && summary.severityCounts
