...
export class RagEngine {
  async search(query: string, options?: { category?: string; maxResults?: number }): Promise<Array<{ title: string; excerpt: string; score: number }>> {
    ...
    const rows = allRows(db, sql, params) as Array<{ title: string; content: string; category: string }>;
  ...
  async generateContext(query: string, maxChunks: number = 5): Promise<string> {
