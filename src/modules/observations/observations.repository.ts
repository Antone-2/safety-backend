...
function asObservation(row: Record<string, unknown>): ObservationRecord {
  return {
    id: String(row.id),
    title: String(row.title),
    type: String(row.type) as ObservationRecord["type"],
    ...
    severity: String(row.severity) as ObservationRecord["severity"],
    status: String(row.status) as ObservationRecord["status"],
...
export class ObservationsRepository {
  ...
  async findAll(filters?: Record<string, unknown>) {
    ...
    const result = await this.pool.query(
      `SELECT * FROM safety_observations ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY observation_date DESC, created_at DESC`,
      params,
    );
    return result.rows.map((row) => asObservation(row as unknown as Record<string, unknown>));
  }

  async findById(id: string) {
    const result = await this.pool.query("SELECT * FROM safety_observations WHERE id = $1", [id]);
    return result.rows[0]
      ? asObservation(result.rows[0] as unknown as Record<string, unknown>)
      : null;
  }

  async create(data: CreateObservationInput) {
    const result = await this.pool.query(
    ...
    return asObservation(result.rows[0] as unknown as Record<string, unknown>);
  }

  async update(id: string, data: UpdateObservationInput) {
    ...
    params.push(id);
    const result = await this.pool.query(
      `UPDATE safety_observations SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`,
      params,
    );
    return result.rows[0]
      ? asObservation(result.rows[0] as unknown as Record<string, unknown>)
