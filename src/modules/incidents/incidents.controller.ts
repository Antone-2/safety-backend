...
import type { IncidentInput } from "./incidents.types.js";
...
export function createIncidentsController(service: IncidentsService) {
  return {
    async getAll(req: AuthRequest, res: Response) {
      ...
      if (String(req.query.status)) filters.status = String(req.query.status);
      if (String(req.query.severity)) filters.severity = String(req.query.severity);
      if (String(req.query.location)) filters.location = String(req.query.location);
      if (String(req.query.department)) filters.department = String(req.query.department);
    ...
    async update(req: AuthRequest, res: Response) {
      ...
      if (incident) {
        await writeAuditLog({
          ...
          resourceType: "incident",
          resourceId: incident.id,
          changes: diffRecord(before as unknown as Record<string, unknown>, incident as unknown as Record<string, unknown>),
          actor: req.user,
          request: req,
        });
      }
    ...
    async transition(req: AuthRequest, res: Response) {
      const incident = await service.getById(String(req.params.id));
      if (!incident) throw new NotFoundError("Incident");
      ...
      const updated = await service.update(String(req.params.id), { status: nextStatus as IncidentInput["status"] });
