...
function asAlert(row: Record<string, unknown>): SafetyAlertRecord {
  return {
    id: String(row.id),
    alertNo: String(row.alert_no),
    title: String(row.title),
    category: String(row.category) as SafetyAlertRecord["category"],
    severity: String(row.severity) as SafetyAlertRecord["severity"],
    status: String(row.status) as SafetyAlertRecord["status"],
...
function asAcknowledgement(row: Record<string, unknown>): SafetyAlertAcknowledgement {
  return {
    id: String(row.id),
...
export class SafetyAlertsRepository {
  ...
  async findAll(filters?: Record<string, unknown>) {
    ...
    const result = await this.pool.query(
      `SELECT * FROM safety_alerts ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY effective_from DESC, created_at DESC`,
      params,
    );
    return result.rows.map((row) => asAlert(row as Record<string, unknown>));
  }

  async findById(id: string) {
    const result = await this.pool.query("SELECT * FROM safety_alerts WHERE id = $1", [id]);
    return result.rows[0] ? asAlert(result.rows[0] as Record<string, unknown>) : null;
  }

  async create(data: CreateSafetyAlertInput) {
    ...
    return asAlert(result.rows[0] as Record<string, unknown>);
  }
  ...
  async update(id: string, data: UpdateSafetyAlertInput) {
    ...
    const result = await this.pool.query(
      `UPDATE safety_alerts SET ${updates.join(", ")} WHERE id = $${idx + 1} RETURNING *`,
      params,
    );
    return result.rows[0] ? asAlert(result.rows[0] as Record<string, unknown>) : null;
  }
  ...
  async acknowledge(alertId: string, input: AcknowledgeSafetyAlertInput) {
    const result = await this.pool.query(
      `INSERT INTO safety_alert_acknowledgements (
    ...
    return asAcknowledgement(result.rows[0] as Record<string, unknown>);
  }

  async getAcknowledgements(alertId: string) {
    const result = await this.pool.query(
      `SELECT * FROM safety_alert_acknowledgements WHERE alert_id = $1 ORDER BY acknowledged_at DESC`,
      [alertId],
    );
    return result.rows.map((row) => asAcknowledgement(row as Record<string, unknown>));
  }

  async getPendingAcknowledgements(userId: string) {
    const result = await this.pool.query(
      `SELECT a.*
  ...
  async getStats(): Promise<SafetyAlertStats> {
