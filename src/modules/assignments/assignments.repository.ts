...
import type { AssignmentRecord, AssignmentStatus } from "./assignments.types.js";

type Queryable = Pick<PoolClient, "query">;

export function mapAssignment(row: Record<string, unknown>): AssignmentRecord {
  return {
    id: String(row.id), reportId: String(row.report_id), status: row.status as AssignmentStatus,
    priority: row.priority as AssignmentRecord["priority"],
...
export class AssignmentsRepository {
  ...
  async findById(id: string, client: Queryable = pgPool): Promise<AssignmentRecord | null> {
  ...
  async list(filters: { email?: string; reportId?: string; status?: string; site?: string; department?: string }) {
    const values: unknown[] = []; const where: string[] = ["a.archived_at IS NULL"];
    ...
    if (filters.reportId) add("a.report_id = ?", filters.reportId);
    if (filters.status) add("a.status = ?", filters.status);
    if (filters.site) add("lower(a.site) = lower(?)", filters.site);
    if (filters.department) add("lower(a.department) = lower(?)", filters.department);
