...
export function createRiskController(service: RiskService) {
  return {
    ...
    async deleteRegister(req: AuthRequest, res: Response) {
      ...
      if (!existing) throw new NotFoundError("Risk register");
      if ("readonly" in (existing as unknown as Record<string, unknown>)) {
        throw new NotFoundError("Risk register");
      }

      const deleted = await service.deleteRegister(String(req.params.id));
      if (!deleted) throw new NotFoundError("Risk register");
      await writeAuditLog({
