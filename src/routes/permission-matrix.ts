...
import { Permission, ROLE_PERMISSIONS } from "../shared/middleware/rbac.middleware.js";

const router = Router();

const PermissionMatrixSchema = z.record(z.array(z.string()));

router.get("/", authenticateUser, requirePermission("users:read"), async (_req: AuthRequest, res: Response) => {
...
router.put("/", authenticateUser, requirePermission("users:update"), async (req: AuthRequest, res: Response) => {
  ...
  for (const [role, permissions] of Object.entries(updates)) {
    ...
    const clean = permissions.filter((p): p is Permission => typeof p === "string" && p.length > 0);
    (ROLE_PERMISSIONS as Record<string, Permission[]>)[role] = clean;
  }
...
router.get("/roles/:role", authenticateUser, requirePermission("users:read"), async (req: AuthRequest, res: Response) => {
...
router.patch("/roles/:role", authenticateUser, requirePermission("users:update"), async (req: AuthRequest, res: Response) => {
  const role = String(req.params.role);
  if (!(role in ROLE_PERMISSIONS)) {
    return res.status(404).json({ error: "Role not found" });
  }
  ...
  if (parsed.data.set) {
    (ROLE_PERMISSIONS as Record<string, Permission[]>)[role] = parsed.data.set.filter(
      (p): p is Permission => typeof p === "string" && p.length > 0,
    );
  } else {
    if (parsed.data.add) {
      for (const perm of parsed.data.add) {
        if (!current.includes(perm as Permission)) current.push(perm as Permission);
      }
    }
    ...
    (ROLE_PERMISSIONS as Record<string, Permission[]>)[role] = current;
  }
