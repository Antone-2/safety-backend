...
import {
  ...
  RiskMatrixLevelSchema,
  type UpdateRiskMatrixInput,
  type RiskMatrixLevelInput,
  type RiskMatrix,
} from "../lib/types.js";

const router = Router();

function parseLevels(value: unknown): RiskMatrixLevelInput[] {
  if (!value) return [];
  if (Array.isArray(value)) return value as RiskMatrixLevelInput[];
  if (typeof value === "string") {
    try { return JSON.parse(value) as RiskMatrixLevelInput[]; } catch { return []; }
  }
  return [];
}

function parseRecord(value: unknown): Record<number, string> {
  if (!value) return {};
  if (typeof value === "object" && !Array.isArray(value)) {
    const result: Record<number, string> = {};
    for (const [k, v] of Object.entries(value as Record<string, unknown>)) {
      const n = Number(k);
      if (!Number.isNaN(n)) result[n] = String(v);
    }
    return result;
  }
  if (typeof value === "string") {
    try { return JSON.parse(value) as Record<number, string>; } catch { return {}; }
  }
  return {};
}

const mapDoc = (data: any): RiskMatrix => ({
  id: data.id,
  name: data.name || "",
...
router.get("/", async (req: Request, res: Response) => {
  const isDefault = String(String(req.query.isDefault)) as string | undefined;

  if (isFirebaseAvailable()) {
    const db = getFirebase()!;
    let query: any = db.collection("risk_matrices");
  ...
  const params: any[] = [];
  if (isDefault !== undefined) { sql += " AND isDefault = ?"; params.push(isDefault === "true" ? 1 : 0); }
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  const db = await getDb();
  const row = db.prepare("SELECT * FROM risk_matrices WHERE id = ?").getAsObject([id]) as any | undefined;
...
router.post("/", async (req: Request, res: Response) => {
  ...
  const normalizedLevels: RiskMatrixLevelInput[] = parsed.data.levels.map((l) => ({
    ...l,
  ...
  const db = await getDb();
  const countRow = db.prepare("SELECT COUNT(*) as c FROM risk_matrices").getAsObject() as { c: number | string | null };
  const count = Number(countRow.c ?? 0);
  const id = `RMT-${String(count + 1).padStart(3, "0")}`;
  const stmt = db.prepare(
    `INSERT INTO risk_matrices (id, name, description, likelihoodScale, severityScale, levels, isDefault, createdBy, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  stmt.run([
    id,
  ...
  const row = db.prepare("SELECT * FROM risk_matrices WHERE id = ?").getAsObject([id]) as any;
  await saveDb(db);
...
router.patch("/:id", async (req: Request, res: Response) => {
  ...
  if (isFirebaseAvailable()) {
    ...
    if (data.likelihoodScale !== undefined) updates.likelihoodScale = data.likelihoodScale;
    if (data.severityScale !== undefined) updates.severityScale = data.severityScale;
  ...
  if (data.likelihoodScale !== undefined) { sets.push("likelihoodScale = ?"); vals.push(JSON.stringify(data.likelihoodScale)); }
  if (data.severityScale !== undefined) { sets.push("severityScale = ?"); vals.push(JSON.stringify(data.severityScale)); }
  if (data.levels !== undefined) {
    ...
    sets.push("levels = ?"); vals.push(JSON.stringify(normalized));
  }
  if (data.isDefault !== undefined) { sets.push("isDefault = ?"); vals.push(data.isDefault ? 1 : 0); }
  vals.push(id);
...
router.post("/:id/levels", async (req: Request, res: Response) => {
  const id = routeParam(req, "id");
  ...
  if (isFirebaseAvailable()) {
    ...
    const doc = await db.collection("risk_matrices").doc(id).get();
    if (!doc.exists) return res.status(404).json({ error: "Not found" });
    const currentLevels = (doc.data() as any).levels || [];
    const newLevel = {
      ...parsed.data,
...
router.delete("/:id/levels/:levelLabel", async (req: Request, res: Response) => {
  const id = routeParam(req, "id");
  ...
  if (isFirebaseAvailable()) {
    ...
    const doc = await db.collection("risk_matrices").doc(id).get();
    if (!doc.exists) return res.status(404).json({ error: "Not found" });
    const currentLevels = ((doc.data() as any).levels || []).filter((l: any) => l.label !== levelLabel);
