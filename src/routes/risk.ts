...
router.get("/matrices", authenticateUser, async (req: AuthRequest, res) => {
  try {
...
router.post("/matrices", authenticateUser, requireRole(["super-admin", "EHS-manager"]), async (req: AuthRequest, res) => {
  try {
...
router.get("/registers", authenticateUser, async (req: AuthRequest, res) => {
  try {
...
router.post("/registers", authenticateUser, requireRole(["super-admin", "EHS-manager", "hse-officer", "plant-manager", "factory-manager"]), async (req: AuthRequest, res) => {
  try {
...
router.get("/registers/:id", authenticateUser, async (req: AuthRequest, res) => {
  try {
    const register = await riskService.getRegisterById(String(String(req.params.id)));
    if (!register) return res.status(404).json({ error: "Risk register not found" });
    res.json(register);
  } catch (error) {
...
router.patch("/registers/:id", authenticateUser, requireRole(["super-admin", "EHS-manager", "hse-officer", "plant-manager", "factory-manager"]), async (req: AuthRequest, res) => {
  try {
...
router.post("/bow-ties", authenticateUser, requireRole(["super-admin", "EHS-manager", "hse-officer"]), async (req: AuthRequest, res) => {
  try {
