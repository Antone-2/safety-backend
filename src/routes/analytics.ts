...
router.get("/dashboards", authenticateUser, async (req: AuthRequest, res) => {
  try {
    const filters: Record<string, any> = {};
    if (String(req.query.type)) filters.type = String(String(req.query.type));
...
router.post(
  "/templates",
  authenticateUser,
  requireRole(analyticsManagers),
  async (req: AuthRequest, res) => {
    try {
      const template = await governanceService.createTemplate(
        req.body,
        req.user,
      );
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      const schedule = await governanceService.createSchedule(
        req.body,
        req.user,
      );
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      const signoff = await governanceService.signoff(
        String(req.params.id),
        req.body,
        req.user,
      );
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      const pack = await governanceService.managementPack(
        "management-review",
        req.user,
      );
