...
router.get("/", authenticateUser, async (req: AuthRequest, res) => {
  try {
    ...
    if (String(req.query.owner)) filters.owner = String(req.query.owner);
...
router.get("/at-risk", authenticateUser, async (_req: AuthRequest, res) => {
  try {
...
router.get("/:id", authenticateUser, async (req: AuthRequest, res) => {
  try {
    ...
    if (!record) return res.status(404).json({ error: "Objective not found" });
    res.json(record);
  } catch (error) {
