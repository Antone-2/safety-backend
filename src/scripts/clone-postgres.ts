...
async function getPublicTables(pool: Pool) {
...
async function getTableColumns(pool: Pool, table: string) {
...
async function getDependencyOrder(pool: Pool, tables: string[]) {
  ...
  for (const row of dependencyResult.rows) {
    if (!tableSet.has(row.table_name) || !tableSet.has(row.depends_on)) continue;
    if (row.table_name === row.depends_on) continue;
  ...
  console.warn(
    `Detected a dependency cycle or unresolved ordering among: ${remaining.join(", ")}. Appending remaining tables alphabetically.`,
  );
  return [...ordered, ...remaining];
}

async function truncateTargetTables(pool: Pool, tables: string[]) {
  if (tables.length === 0) return;
  const sql = `TRUNCATE TABLE ${tables.map((table) => quoteIdentifier(table)).join(", ")} RESTART IDENTITY CASCADE`;
  await pool.query(sql);
}

async function loadTableInfo(pool: Pool, tables: string[]): Promise<TableInfo[]> {
...
function buildInsertSql(table: TableInfo, rowCount: number) {
...
async function copyTable(source: Pool, target: Pool, table: TableInfo, batchSize = 200) {
...
async function syncSequences(target: Pool) {
