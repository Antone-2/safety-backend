...
const DEFAULT_MIN_SUCCESS_RATE_PERCENT = Number(
  process.env.DASHBOARD_BENCH_MIN_SUCCESS_RATE_PERCENT || 99.5,
);
...
const DEFAULT_MIN_REQUESTS_PER_SECOND = Number(
  process.env.DASHBOARD_BENCH_MIN_RPS || 100,
);
...
function readOptionalNumber(name: string, fallback?: number): number | undefined {
  const raw = readArg(name);
...
function formatThresholdResult(label: string, passed: boolean, actual: number, expected: string) {
  return {
    label,
    passed,
    actual: round(actual),
...
async function resolveToken(baseUrl: string): Promise<string> {
  ...
  if (!email || !password) {
    throw new Error(
      "Provide DASHBOARD_BENCH_TOKEN or DASHBOARD_BENCH_EMAIL and DASHBOARD_BENCH_PASSWORD.",
    );
  }
  ...
  if (payload.requiresMfa || payload.mfaRequired) {
    throw new Error(
      "Login requires MFA. Use DASHBOARD_BENCH_TOKEN from an authenticated session.",
    );
  }
...
async function main() {
  ...
  const thresholds: BenchmarkThresholds = {
    minSuccessRatePercent: readOptionalNumber(
      "min-success-rate",
    ...
    minRequestsPerSecond: readOptionalNumber(
      "min-rps",
      DEFAULT_MIN_REQUESTS_PER_SECOND,
    ),
  };
  ...
  const gateResults = [
    thresholds.minSuccessRatePercent == null
      ...
      : formatThresholdResult(
          "successRatePercent",
          summary.totals.successRatePercent >= thresholds.minSuccessRatePercent,
          summary.totals.successRatePercent,
          `>= ${thresholds.minSuccessRatePercent}`,
        ),
    ...
    thresholds.minRequestsPerSecond == null
      ...
      : formatThresholdResult(
          "requestsPerSecond",
          summary.totals.requestsPerSecond >= thresholds.minRequestsPerSecond,
          summary.totals.requestsPerSecond,
