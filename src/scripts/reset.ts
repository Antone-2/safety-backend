...
async function main() {
  ...
  if (seedPasswordSuper && seedPasswordSuper.length >= 12) {
    ...
    await pgPool.query(
      ...
      ["admin@crownpaints.co.ke", hash, "Super Admin", "super-admin"],
    );
  ...
  if (seedPasswordEhs && seedPasswordEhs.length >= 12) {
    ...
    await pgPool.query(
      `INSERT INTO users (email, password_hash, name, role)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email) DO NOTHING`,
      ["safety@crownpaints.co.ke", hash, "EHS Manager", "EHS-manager"],
    );
...
main().catch((error) => {
  logger.error({ err: error as Error }, "Database reset failed.");
  process.exitCode = 1;
}).finally(async () => {
  await pgPool.end();
});
