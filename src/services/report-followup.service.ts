...
function stageLabel(stage: FollowupStage): string {
  switch (stage) {
    case "reminder":
      return "Reminder";
    case "urgent":
...
function stageMessage(stage: FollowupStage, dueAt: string): string {
...
function resolveRecipients(report: ReportFollowup): { channel: "email"; recipient: string; name?: string }[] {
  const recipients: { channel: "email"; recipient: string; name?: string }[] = [];
...
async function alreadyNotifiedRecently(reportId: string, stage: FollowupStage): Promise<boolean> {
  const result = await pgPool.query(
    `SELECT created_at FROM notification_jobs
     WHERE resource_type = 'report' AND resource_id = $1 AND event_key = $2
     ORDER BY created_at DESC LIMIT 1`,
    [reportId, `${FOLLOWUP_EVENT_KEY}:${stage}`],
  );
  if (!result.rows[0]?.created_at) return false;
...
export async function findReportsNeedingFollowup(limit = 50): Promise<ReportFollowup[]> {
  if (!pgPool) return [];
  const now = new Date();

  const result = await pgPool.query(
    `SELECT id, status, severity, sla_hours, due_at, assigned_to, assigned_to_copy,
  ...
  const candidates: ReportFollowup[] = [];
  for (const row of result.rows) {
    const status = String(row.status ?? "");
    const assignedTo = String(row.assigned_to ?? "").trim();
    if (status === "Closed" || !assignedTo) continue;
    ...
    const assignedToCopy = Array.isArray(row.assigned_to_copy)
      ? row.assigned_to_copy.map((v: unknown) => String(v))
      : [];
...
export async function enqueueFollowup(report: ReportFollowup): Promise<void> {
  if (!pgPool) return;
  if (await alreadyNotifiedRecently(report.reportId, report.stage)) return;
...
export async function scheduleFollowupsForReport(reportId: string): Promise<void> {
  if (!pgPool) return;
  const result = await pgPool.query(
    `SELECT id, status, severity, sla_hours, due_at, assigned_to, assigned_to_copy,
            location, description
  ...
  if (!row || row.status === "Closed" || !row.assigned_to) return;
