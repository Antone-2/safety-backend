...
function mapRow(row: Record<string, unknown>): InvestigationRecord {
  return {
    ...
    status: String(row.status ?? "Pending") as InvestigationStatus,
    priority: String(row.priority ?? "Medium") as InvestigationPriority,
...
export class InvestigationService {
  private validate(data: InvestigationInput) {
    return InvestigationSchema.parse(data);
  }

  async getAll(filters?: Record<string, any>): Promise<InvestigationRecord[]> {
    const where: string[] = [];
  ...
  async update(id: string, data: Record<string, any>): Promise<InvestigationRecord | null> {
    ...
    const result = await pgPool.query(
      `UPDATE investigations SET ${fields.join(", ")} WHERE id = $${params.length} RETURNING *`,
      params,
    );
    return result.rows[0] ? mapRow(result.rows[0] as Record<string, unknown>) : null;
  }
