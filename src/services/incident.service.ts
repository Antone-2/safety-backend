...
export class IncidentService extends BaseService {
  ...
  async getOverdue() {
    const result = await pgPool.query(
      `SELECT * FROM ${this.tableName} WHERE status != 'Closed' AND "dueAt" < $1`,
      [now()],
    );
    return result.rows;
  }
  ...
  async getCriticalOpen() {
    const result = await pgPool.query(
      `SELECT * FROM ${this.tableName} WHERE severity = 'Critical' AND status != 'Closed'`,
    );
    return result.rows;
  }
  ...
  async getStats() {
    ...
    const todayResult = await pgPool.query(
      `SELECT COUNT(*) as count FROM ${this.tableName} WHERE "createdAt" >= NOW()::date`,
    );
    const today = Number(todayResult.rows[0]?.count ?? 0);
    const weekResult = await pgPool.query(
      `SELECT COUNT(*) as count FROM ${this.tableName} WHERE "createdAt" >= NOW() - INTERVAL '7 days'`,
    );
