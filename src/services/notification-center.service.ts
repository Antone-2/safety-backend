...
export class NotificationCenterService {
  async listTemplates() {
    const result = await pgPool.query(
      "SELECT * FROM notification_templates ORDER BY event_key, channel",
    );
    return result.rows.map(templateRow);
  }

  async upsertTemplate(data: Record<string, any>, actor?: { name?: string; email?: string }) {
    const result = await pgPool.query(
    ...
    try {
      ...
      for (const recipient of input.recipients) {
        await client.query(
          ...
          [randomUUID(), result.rows[0].id, recipient.channel, recipient.recipient, recipient.name || null],
        );
      }
  ...
  async processJob(jobId: string) {
    ...
    const [recipientResult, templateResult] = await Promise.all([
      pgPool.query(
        "SELECT * FROM notification_recipients WHERE job_id = $1 AND status IN ('queued','retrying','failed')",
        [jobId],
      ),
      pgPool.query(
        "SELECT * FROM notification_templates WHERE event_key = $1 AND active = TRUE",
        [job.eventKey],
      ),
    ]);
    ...
    for (const rawRecipient of recipientResult.rows) {
      ...
      const body = renderTemplate(
        template?.bodyTemplate || String(job.payload.message || ""),
        job.payload,
      );
  ...
  async listJobs(filters?: { status?: string; limit?: number }) {
    ...
    const result = await pgPool.query(
      `SELECT * FROM notification_jobs ${where} ORDER BY created_at DESC LIMIT $${values.length}`,
      values,
    );
  ...
  async listDigests(filters?: { userId?: string; recipient?: string }) {
    ...
    const result = await pgPool.query(
      `SELECT * FROM notification_digest_subscriptions ${where} ORDER BY created_at DESC`,
      values,
    );
    return result.rows.map(this.mapDigestRow);
  }

  async updateDigest(id: string, input: { cadence?: string; channels?: NotificationChannel[]; active?: boolean }) {
    const sets: string[] = [];
    const values: unknown[] = [];
    ...
    values.push(id);
    const result = await pgPool.query(
      `UPDATE notification_digest_subscriptions SET ${sets.join(", ")} WHERE id = $${values.length} RETURNING *`,
      values,
    );
    const row = result.rows[0];
    if (!row) throw new Error("Digest subscription not found");
    return this.mapDigestRow(row);
  }
  ...
  async deleteDigest(id: string) {
    ...
    if (result.rowCount === 0) throw new Error("Digest subscription not found");
    return { ok: true };
  }

  private mapDigestRow(row: any) {
    return {
      id: row.id,
      userId: row.user_id,
      recipient: row.recipient,
  ...
  private async deliver(channel: string, recipient: string, subject: string, body: string) {
    if (channel === "in-app") return { delivered: true, providerMessageId: "in-app" };
    if (channel === "email") {
      try {
